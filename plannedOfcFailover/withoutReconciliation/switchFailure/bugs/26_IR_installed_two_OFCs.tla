...
(************************* variables ***************************************)
    variables (*************** Some Auxiliary Vars ***********************)
              ...
              flowStatReqStatus = [x \in {ofc0, ofc1} |-> [y \in SW |-> IR_NONE]],
              ...
              ofcModuleTerminationStatus = [x \in {ofc0, ofc1} |-> 
                                                [y \in ({CONT_MONITOR, CONT_EVENT, NIB_EVENT_HANDLER} \cup CONTROLLER_THREAD_POOL) 
                                                                                                                            |-> TERMINATE_NONE]],
              \* ofcModuleInitStatus used for NIB event handler to wake up a process 
              ofcModuleInitStatus = (ofc0 :> [x \in {CONT_EVENT} |-> INIT_MASTER]) @@ (ofc1 :> [x \in {CONT_EVENT} |-> INIT_PROCESS]),
              \* recoveredSwitches is a set containing all the switches with slave role used by Failover handler to reschedule ROLE_REQ
              setRecoveredSw = [x \in {ofc0, ofc1} |-> {}],
    ...
    define
        ...
        shouldNIBEventHandlerTerminate(CID) == ofcModuleTerminationStatus[CID][NIB_EVENT_HANDLER] = TERMINATE_INIT 
        ...
        existRecoveredSwitchWithSlaveRole(CID) == setRecoveredSw[CID] # {}
        ...
        workerCanSendTheIR(CID, nextToSent) == /\ ~isSwitchSuspended(CID, nextToSent.to)
                                               /\ \/ /\ nextToSent.type = ROLE_REQ
                                                  ...
                                                  \/ /\ nextToSent.type = INSTALL_FLOW
                                                     ...
                                                     /\ ofcSwSuspensionStatus[CID][nextToSent.to] # SW_RECONCILE
        ...
        shouldSendFlowStatReqForAllEntriesOnReconciliation(monitoringEvent, tid) == /\ NIBSwSuspensionStatus[monitoringEvent.swID] = SW_RECONCILE
                                                                                    /\ ~existsMonitoringEventHigherNum(monitoringEvent, tid)
                                                                                    /\ Len(eventSkipList[tid][monitoringEvent.swID]) <= 1 
        shouldPerformDataPlaneReconciliation(swID, tid, remainingEvents) == /\ NIBSwSuspensionStatus[swID] = SW_RECONCILE 
                                                                            /\ eventSkipList[tid][swID] = <<>> 
                                                                            /\ remainingEvents = <<>>
    ...
    begin
        ofcSwSuspensionStatus[self[1]][swID] := SW_RECONCILE;
        if masterState[self[1]] = ROLE_MASTER then
            NIBSwSuspensionStatus[swID] := SW_RECONCILE;
        end if; 
    ...
    while TRUE do
        ...
        if ingressPkt.type = INSTALL_FLOW then 
            ...
            if ingressPkt.IR = ALL_IR then
                sendFlowStatReplyAllEntries(ingressPkt.from);
            elsif existMatchingEntryTCAM(self[2], ingressPkt.IR) then
    ...
    while TRUE do
        ...
        \*   2.4. if it is FLOW_STAT_REQ msg, check whether there is a matching entry in the TCAM or not. 
        SwitchOfaProcessPacket:
           if swOFACanProcessIRs(self[2]) then
                ...
                if ofaInMsg.type = INSTALL_FLOW then
                    ...
                    assert \/ ofaInMsg.IR = ALL_IR 
                           \/ ofaInMsg.IR \in 1..MAX_NUM_IRs;
                    switchAcquireLock();
                    if ofaInMsg.IR = ALL_IR then
                        sendFlowStatReplyAllEntries(ofaInMsg.from);
                    elsif existMatchingEntryTCAM(self[2], ofaInMsg.IR) then
    ...
    begin
    NibEventHandlerProc:
        while ~shouldNIBEventHandlerTerminate(self[1]) do
            controllerReleaseLocalLock();
            ...
            assert event.type \in {INSTALL_FLOW};
            ...
            \*             setRecoveredSw[self[1]] := setRecoveredSw[self[1]] \cup {event.sw};
            \*         end if;
            \*    end if;
    ...
    ofcFailoverHandlerProc:
        while TRUE do
            ...
            \* 2) subscribe to NIB IR Queue
            \* 3) pull the IRs from NIB IRQueue
            \* 4) change its failover status to FAILOVER_READY
            \* 5) wait for OFC to have role master in NIB
            \* 6) reconcile its local worker queue with NIB IR queue
            \* 7) initialize OFC controller event handler
            if ofcFailoverStateNIB[self[1]] = FAILOVER_INIT then 
                ...
                \* Step --. subscribe to and pull NIB switch suspension status
               ...
               QueryIRQueueNIB:
                    while fetchedIRsBeforePassingToWorker[self[1]] # <<>> do
                        \* Step 5: wait for the OFC to have role master in NIB
                        controllerReleaseLocalLock();
                        controllerAcquireGlobalLock();
                        await masterState[self[1]] = ROLE_MASTER;
                        \* Step 6: add IRs to worker queue
                    ...
                    \* Step 7: initalize OFC controller event handler
                    controllerReleaseLocalLock();
                    controllerAcquireGlobalLock();
                    await masterState[self[1]] = ROLE_MASTER;
               ...
               ReconcileEventLogs:
                    while swSet # {} do
                        ...
                        ReconcileEventForSW:
                            ...
                            if receivedEventsCopy[currSW] # <<>> then
                                ...
                                \* pull the switch state from NIB to local
                                ofcSwSuspensionStatus[self[1]][currSW] := NIBSwSuspensionStatus[currSW];
                            elsif pulledNIBEventLog[currSW] # <<>> then
                                controllerReleaseLocalLock();
                                controllerReleaseGlobalLock();
                                eventSkipList[self[1]][currSW] := eventSkipList[self[1]][currSW] \o pulledNIBEventLog[currSW];
                            elsif shouldPerformDataPlaneReconciliation(currSW, self[1], remainingEvents) /\ latestRecoveryEvent.type # 0 then
                                controllerReleaseLocalLock();
                                controllerAcquireGlobalLock();
                                ofcSwSuspensionStatus[self[1]][currSW] := SW_SUSPEND;
                                eventSkipList[self[1]][currSW] := Append(eventSkipList[self[1]][currSW], latestRecoveryEvent);
                                remainingEvents := Append(remainingEvents, latestRecoveryEvent);
                                latestRecoveryEvent := [type |-> 0];
                            end if;
                            swSet := swSet \ {currSW};
                    end while;
                    ...
                    ofcModuleInitStatus[self[1]][CONT_EVENT] := INIT_MASTER; 
            ...
            \* 3) change its failover status to FAILOVER_TERMINATE_DONE       
              ...
              WaitForWorkersTermination:
                    ...
                    ofcModuleTerminationStatus[self[1]] := [y \in {CONT_MONITOR, NIB_EVENT_HANDLER} |-> TERMINATE_INIT] 
                                                                                @@ ofcModuleTerminationStatus[self[1]];
                ...
                if idThreadWorkingOnIR[self[1]][entryIndex] = IR_UNLOCK then
                    ...
                    if (stepOfFailure = 3) then
                        ...
                        if (workerCanSendTheIR(self[1], nextToSent)) then
                            ...
                            assert nextToSent.type \in {INSTALL_FLOW, ROLE_REQ, FLOW_STAT_REQ};
                            ...
                            elsif nextToSent.type = FLOW_STAT_REQ then 
                                flowStatReqStatus[self[1]][nextToSent.to] := IR_SENT;
                            end if;
    ...
    ControllerThreadStateReconciliation:
        ...
        if (controllerStateNIB[self].type = STATUS_LOCKING) then
            if (controllerStateNIB[self].next.type = INSTALL_FLOW) then
                ...
                if (flowStatReqStatus[self[1]][controllerStateNIB[self].next.to] = IR_SENT) then
                    flowStatReqStatus[self[1]][controllerStateNIB[self].next.to] := IR_NONE;
                end if;
    ...
    while ~shouldEventHandlerTerminate(self[1]) do 
         ...
         \* If OF Init status is INIT_NONE, OFC OF event handler waits until it becomes either
         \* INIT_MASTER or INIT_PROCESS
         assert ofcModuleInitStatus[self[1]][self[2]] \in {INIT_NONE, INIT_MASTER, INIT_PROCESS};
         await ofcModuleInitStatus[self[1]][self[2]] \in {INIT_PROCESS, INIT_MASTER};
         controllerWaitForLocalLockFree();
         controllerWaitForGlobalLockFree();
         \* Controller event handler process consists of two operations;
         \* 1. Picking the first event from the event queue
         \* 2. Check whether the eventSkipList is empty or not.  
         \*     2.1. If not empty, 
         \*             check whether the current event is equivalent to the head of skip list
         \*         2.1.1. If it is, process the event internally without changing anything in NIB
         \*         2.2.2. If it is not, do nothing. 
         \*         Finally, remove the head of the skipList and start the process from the begining.  
         \*     2.2. If empty, 
         \*             check whether the event is a switch failure or a switch recovery
         \*         2.2.1 if a switch failure and current state of switch is SW_RUN
         \*                 , then suspend the switch both locally and in NIB
         \*         2.2.2 if a switch recovery and current state of switch is SW_SUSPEND
         \*                 , then  
            ...
            if eventSkipList[self[1]][monitoringEvent.swID] # <<>> then
               if areEventsEquivalent(monitoringEvent, Head(eventSkipList[self[1]][monitoringEvent.swID])) then
                    ...
                    if shouldSuspendSw(monitoringEvent) /\ ofcSwSuspensionStatus[self[1]][monitoringEvent.swID] = SW_RUN then
                        ...
                        controllerReleaseGlobalLock();
                        if shouldSendFlowStatReqForAllEntriesOnReconciliation(monitoringEvent, self[1]) then
               ...
               goto ControllerEventHandlerProc;
                ...
                if shouldSuspendSw(monitoringEvent) /\ ofcSwSuspensionStatus[self[1]][monitoringEvent.swID] = SW_RUN then
                    ...
                    if (controllerSubmoduleFailStat[self] = NotFailed) then
                        controllerReleaseLocalLock();
                        if (ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER) then
                            controllerReleaseGlobalLock();
                            if (masterState[self[1]] = ROLE_MASTER) then
                                isEventProcessed := 1;
                    ...
                    \* 5. Reconcile the state of topology (NIB's and local's) with the dataplane
                    whichStepToFail(3);
                    if (stepOfFailure = 1) then 
                        controllerModuleFails();
                        goto ControllerEventHandlerStateReconciliation;
                        ...
                        if (stepOfFailure = 2) then
                            ...
                            setRecoveredSw[self[1]] := setRecoveredSw[self[1]] \cup {monitoringEvent.swID}; 
                            if (ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER) then 
                                isEventProcessed := 1; 
                                if (stepOfFailure = 3) then
                                    controllerModuleFails();
                                    ...
                                    if ~existsMonitoringEventHigherNum(monitoringEvent, self[1]) then
                                        ...
                                        \* Step 5: reconcile the state of topology (NIB's and local's) with the data plane
                                        ScheduleFlowStatReq:
                                            controllerReleaseGlobalLock();
                                            controllerReleaseLocalLock();
                                            scheduleFlowStatReqAllEntries(monitoringEvent.swID); 
    ...
    variable msg = [type |-> 0], stepOfFailure = 0, setIRs = {}, currIR = 0;
    begin
    ...
    while ~shouldMonitoringServerTerminate(self[1]) do
        ...
        \***** 2.3. type = ROLE_REPLY -> sw's responce to the role req msg. 
        \* 3. Remove the packet from queue (Lazy Removal).
        ...
        controllerAcquireGlobalLock();
        \* Step 1. Read the first packet from the queue. 
        ...
        if msg.type = INSTALLED_SUCCESSFULLY then
            ...
            controllerReleaseLocalLock(); 
            assert msg.status \in {ALL_IR, NO_ENTRY, ENTRY_FOUND};
            assert ofcSwSuspensionStatus[self[1]][msg.from] \in {SW_RECONCILE};
            flowStatReqStatus[self[1]][msg.from] := IR_DONE;
            ofcSwSuspensionStatus[self[1]][msg.from] := SW_RUN;
                    
            if masterState[self[1]] = ROLE_MASTER then
                \* only master OFC has write access to NIB SwSuspensionStatus
                NIBSwSuspensionStatus[msg.from] := SW_RUN
            end if;
            
            if msg.status = ALL_IR then
                ...
                changeStatusOfIRs:
                    while TRUE do \* -- while setIRs # {} do --
                        ...
                        if currIR \in msg.IR then
                            IRStatus[currIR] := IR_DONE;
                        else
                            IRStatus[currIR] := IR_NONE;
                        end if;
                        
                        if setIRs = {} then 
                            goto MonitoringServerLazyRemoveFromQueue;
                        end if;
                    end while;
                            
            elsif msg.status = ENTRY_FOUND then
                IRStatus[msg.IR] := IR_DONE;
            else \* when msg.status = NO_ENTRY
                IRStatus[msg.IR] := IR_NONE;
            end if;
                         
       elsif msg.type = ROLE_REPLY then
            \* if msg type is ROLE_REPLY, we have to update the controller view of its role in the corresponding
            \* sw.
            controllerAcquireLocalLock();
            if (stepOfFailure # 1) then
        ...
        MonitoringServerLazyRemoveFromQueue:
            controllerReleaseGlobalLock();
            controllerReleaseLocalLock();
            if (stepOfFailure # 2) then
    ...
    \* consists of 3 steps; 
    \* 1) Sending FAILOVER_INIT to OFC1 and waiting to receive FAILOVER_READY
    \* 2) Sending FAILOVER_TERMINATE and get FAILOVER_TERMINATE_DONE from ofc0
    \* 3) change the mastership of OFCs
    begin 
...
*)
\* BEGIN TRANSLATION - the hash of the PCal code: PCal-01b781e4616831890bccf0289e3b3477
\* Process variable controllerSet of process swFailureProc at line 1543 col 76 changed to controllerSet_
\* Process variable nxtController of process swFailureProc at line 1544 col 5 changed to nxtController_
\* Process variable prevLockHolder of process swFailureProc at line 1544 col 25 changed to prevLockHolder_
\* Process variable controllerSet of process swResolveFailure at line 1629 col 83 changed to controllerSet_s
\* Process variable nxtController of process swResolveFailure at line 1630 col 5 changed to nxtController_s
\* Process variable stepOfFailure of process controllerSequencer at line 1772 col 50 changed to stepOfFailure_
\* Process variable event of process nibEventHandler at line 1885 col 15 changed to event_
\* Process variable stepOfFailure of process controllerWorkerThreads at line 2119 col 29 changed to stepOfFailure_c
\* Process variable stepOfFailure of process controllerEventHandler at line 2345 col 80 changed to stepOfFailure_co
VARIABLES switchLock, controllerLocalLock, controllerGlobalLock, FirstInstall, 
          ...
          ofcModuleTerminationStatus, ofcModuleInitStatus, setRecoveredSw, 
...
VARIABLES ingressPkt, ingressIR, egressMsg, ofaInMsg, ofaOutConfirmation, 
          ...
          receivedEventsCopy, latestRecoveryEvent, nextToSent, entryIndex, 
          rowIndex, rowRemove, removeRow, stepOfFailure_c, monitoringEvent, 
          setIRsToReset, resetIR, stepOfFailure_co, notifOFC, 
          isEventProcessed, msg, stepOfFailure, setIRs, currIR, 
...
vars == << switchLock, controllerLocalLock, controllerGlobalLock, 
           ...
           ofcModuleInitStatus, setRecoveredSw, 
           fetchedIRsBeforePassingToWorker, eventSkipList, masterState, 
           controllerStateNIB, IRStatus, NIBSwSuspensionStatus, IRQueueNIB, 
           subscribeList, SetScheduledIRs, ofcFailoverStateNIB, NIBEventLog, 
           pc, ingressPkt, ingressIR, egressMsg, ofaInMsg, ofaOutConfirmation, 
           installerInIR, statusMsg, notFailedSet, failedElem, controllerSet_, 
           nxtController_, prevLockHolder_, failedSet, statusResolveMsg, 
           recoveredElem, controllerSet_s, nxtController_s, prevLockHolder, 
           eventMsg, controllerSet, nxtController, eventID, toBeScheduledIRs, 
...
Init == (* Global variables *)
        ...
        /\ latestRecoveryEvent = [self \in ({ofc0, ofc1} \X {FAILOVER_HANDLER}) |-> [type |-> 0]]
        ...
        /\ setIRs = [self \in ({ofc0, ofc1} \X {CONT_MONITOR}) |-> {}]
        /\ currIR = [self \in ({ofc0, ofc1} \X {CONT_MONITOR}) |-> 0]
        (* Process watchDog *)
...
SwitchSimpleProcess(self) == /\ pc[self] = "SwitchSimpleProcess"
                             ...
                             /\ Assert(ingressPkt'[self].type \in {INSTALL_FLOW, ROLE_REQ, FLOW_STAT_REQ}, 
                                       "Failure of assertion at line 1274, column 9.")
                             /\ IF ingressPkt'[self].type = INSTALL_FLOW
                                   ...
                                   ELSE /\ IF ingressPkt'[self].type = ROLE_REQ
                                              THEN /\ Assert(ingressPkt'[self].roletype \in {ROLE_MASTER, ROLE_SLAVE, ROLE_EQUAL}, 
                                                             "Failure of assertion at line 1284, column 13.")
                             ...
                             /\ Assert(\/ switchLock[2] = self[2]
                                       \/ switchLock[2] = NO_LOCK, 
                                       "Failure of assertion at line 1009, column 9 of macro called at line 1299, column 9.")
                             /\ switchLock' = <<NO_LOCK, NO_LOCK>>
                             ...
                             /\ UNCHANGED << controllerLocalLock, 
                                             ...
                                             remainingEvents, 
                                             receivedEventsCopy, 
                                             latestRecoveryEvent, nextToSent, 
...
SwitchRcvPacket(self) == /\ pc[self] = "SwitchRcvPacket"
                         ...
                         /\ Assert(ingressIR'[self].type \in {ROLE_REQ, INSTALL_FLOW, FLOW_STAT_REQ}, 
                                   "Failure of assertion at line 1324, column 9.")
                         /\ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                         /\ \/ switchLock \in {<<NO_LOCK, NO_LOCK>>, self}
                            \/ /\ switchLock[1] \notin {SW_FAILURE_PROC, SW_RESOLVE_PROC}
                               /\ switchLock[2] = self[2]
...
SwitchFromOFAPacket(self) == /\ pc[self] = "SwitchFromOFAPacket"
                             ...
                             /\ Assert(egressMsg'[self].type \in {INSTALLED_SUCCESSFULLY, ROLE_REPLY, BAD_REQUEST, FLOW_STAT_REPLY}, 
                                       "Failure of assertion at line 1352, column 9.")
...
SwitchNicAsicSendOutMsg(self) == /\ pc[self] = "SwitchNicAsicSendOutMsg"
                                 /\ IF swCanReceivePackets(self[2])
                                       THEN /\ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                            ...
                                            /\ Assert(\/ switchLock[2] = self[2]
                                                      \/ switchLock[2] = NO_LOCK, 
                                                      "Failure of assertion at line 1009, column 9 of macro called at line 1358, column 17.")
                                            /\ switchLock' = <<NO_LOCK, NO_LOCK>>
...
SwitchOfaProcIn(self) == /\ pc[self] = "SwitchOfaProcIn"
                         ...
                         /\ Assert(ofaInMsg'[self].to = self[2], 
                                   "Failure of assertion at line 1389, column 9.")
                         /\ Assert(\/ /\ ofaInMsg'[self].type = ROLE_REQ
                                   ...
                                   \/ /\ ofaInMsg'[self].type = FLOW_STAT_REQ
                                      /\ \/ ofaInMsg'[self].IR = ALL_IR
                                         \/ ofaInMsg'[self].IR \in 1..MAX_NUM_IRs, 
                                   "Failure of assertion at line 1390, column 9.")
                         /\ Assert(ofaInMsg'[self].from \in {ofc0, ofc1}, 
                                   "Failure of assertion at line 1397, column 9.")
                                      ...
                                      THEN /\ IF ofaInMsg[self].type = INSTALL_FLOW
                                                 ...
                                                 ELSE /\ IF ofaInMsg[self].type = ROLE_REQ
                                                            THEN /\ Assert(ofaInMsg[self].roletype \in {ROLE_MASTER, ROLE_SLAVE, ROLE_EQUAL}, 
                                                                           "Failure of assertion at line 1430, column 21.")
                                                                 /\ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                 /\ \/ switchLock \in {<<NO_LOCK, NO_LOCK>>, self}
                                                                    \/ /\ switchLock[1] \notin {SW_FAILURE_PROC, SW_RESOLVE_PROC}
                                                                       /\ switchLock[2] = self[2]
                                                            ...
                                                            ELSE /\ IF ofaInMsg[self].type = FLOW_STAT_REQ
                                                                       THEN /\ Assert(\/ ofaInMsg[self].IR = ALL_IR
                                                                                      \/ ofaInMsg[self].IR \in 1..MAX_NUM_IRs, 
                                                                                      "Failure of assertion at line 1449, column 21.")
                                                                            /\ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                            /\ \/ switchLock \in {<<NO_LOCK, NO_LOCK>>, self}
                                                                               \/ /\ switchLock[1] \notin {SW_FAILURE_PROC, SW_RESOLVE_PROC}
                                                                                  /\ switchLock[2] = self[2]
                                                                            /\ switchLock' = self
                                                                            /\ IF ofaInMsg[self].IR = ALL_IR
                                                                                  THEN /\ Ofa2NicAsicBuff' = [Ofa2NicAsicBuff EXCEPT ![self[2]] = Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                                                                                                                                    from |-> self[2],
                                                                                                                                                                                    to |-> (ofaInMsg[self].from),
                                                                                                                                                                                    status |-> ALL_IR,
                                                                                                                                                                                    IR |-> rangeSeq(TCAM[self[2]])])]
                                                                                  ELSE /\ IF existMatchingEntryTCAM(self[2], ofaInMsg[self].IR)
                                                                                             THEN /\ Ofa2NicAsicBuff' = [Ofa2NicAsicBuff EXCEPT ![self[2]] = Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                                                                                                                                               from |-> self[2],
                                                                                                                                                                                               to |-> (ofaInMsg[self].from),
                                                                                                                                                                                               status |-> ENTRY_FOUND,
...
SwitchOfaProcOut(self) == /\ pc[self] = "SwitchOfaProcOut"
                          ...
                          /\ Assert(ofaOutConfirmation'[self].IR \in 1..MAX_NUM_IRs, 
                                    "Failure of assertion at line 1479, column 9.")
...
SwitchInstallerProc(self) == /\ pc[self] = "SwitchInstallerProc"
                             ...
                             /\ Assert(installerInIR'[self].IR \in 1..MAX_NUM_IRs, 
                                       "Failure of assertion at line 1515, column 8.")
...
SwitchFailure(self) == /\ pc[self] = "SwitchFailure"
                       ...
                       /\ Assert(statusMsg[self].type = 0, 
                                 "Failure of assertion at line 1559, column 9.")
                       /\ notFailedSet'[self] # {}
                       /\ ~isFinished(self[2])
                       ...
                       /\ Assert((self[2]) \in Head(sw_fail_ordering_var), 
                                 "Failure of assertion at line 637, column 9 of macro called at line 1566, column 9.")
                       ...
                       /\ IF failedElem'[self] = "cpu"
                             THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                  /\ Assert(switchStatus[self[2]].cpu = NotFailed, 
                                            "Failure of assertion at line 724, column 9 of macro called at line 1575, column 17.")
                             ...
                             ELSE /\ IF failedElem'[self] = "ofa"
                                        THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                             /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].ofa = NotFailed, 
                                                       "Failure of assertion at line 783, column 9 of macro called at line 1578, column 17.")
                                        ...
                                        ELSE /\ IF failedElem'[self] = "installer"
                                                   THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                                        /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].installer = NotFailed, 
                                                                  "Failure of assertion at line 834, column 9 of macro called at line 1581, column 17.")
                                                        ...
                                                        /\ IF switchStatus'[self[2]].nicAsic = NotFailed /\ switchStatus'[self[2]].ofa = NotFailed
                                                              THEN /\ Assert(switchStatus'[self[2]].installer = Failed, 
                                                                             "Failure of assertion at line 838, column 13 of macro called at line 1581, column 17.")
                                                                   /\ auxEventCounter' = auxEventCounter + 1
                                                                   /\ switchGeneratedEventSet' = [switchGeneratedEventSet EXCEPT ![self[2]] = switchGeneratedEventSet[self[2]] \cup {auxEventCounter'}]
                                                                   /\ controlMsgCounter' = [controlMsgCounter EXCEPT ![self[2]] = controlMsgCounter[self[2]] + 1]
                                                   ...
                                                   ELSE /\ IF failedElem'[self] = "nicAsic"
                                                              THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                                                   /\ Assert(switchStatus[self[2]].nicAsic = NotFailed, 
                                                                             "Failure of assertion at line 661, column 9 of macro called at line 1584, column 17.")
                                                                   ...
                                                                   /\ statusMsg' = [statusMsg EXCEPT ![self] = [type |-> NIC_ASIC_DOWN,
                                                                                                                ...
                                                                                                                auxNum |-> auxEventCounter']]
                                                              ELSE /\ Assert(FALSE, 
                                                                             "Failure of assertion at line 1585, column 14.")
                                                                   /\ UNCHANGED << controller2Switch, 
                                                                                   switchStatus, 
                                                                                   swFailedNum, 
                                                                                   controlMsgCounter, 
                       ...
                       /\ IF shouldSendFailMsgToAll(statusMsg'[self])
                             ...
                             ELSE /\ IF statusMsg'[self].type # 0
                                        THEN /\ Assert(FALSE, 
                                                       "Failure of assertion at line 1618, column 13.")
                                        ELSE /\ TRUE
                       ...
                       /\ UNCHANGED << controllerLocalLock, 
                                       ...
                                       stepOfFailure_co, notifOFC, 
                                       isEventProcessed, msg, stepOfFailure, 
                                       setIRs, currIR, controllerFailedModules >>
...
SwitchResolveFailure(self) == /\ pc[self] = "SwitchResolveFailure"
                              /\ Assert(statusResolveMsg[self].type = 0, 
                                        "Failure of assertion at line 1636, column 9.")
                              ...
                              /\ IF recoveredElem'[self] = "cpu"
                                    THEN /\ ofaStartingMode(self[2]) /\ installerInStartingMode(self[2])
                                         /\ Assert(switchStatus[self[2]].cpu = Failed, 
                                                   "Failure of assertion at line 755, column 9 of macro called at line 1647, column 39.")
                                    ...
                                    ELSE /\ IF recoveredElem'[self] = "nicAsic"
                                               THEN /\ nicAsicStartingMode(self[2])
                                                    /\ Assert(switchStatus[self[2]].nicAsic = Failed, 
                                                              "Failure of assertion at line 691, column 9 of macro called at line 1648, column 46.")
                                               ...
                                               ELSE /\ IF recoveredElem'[self] = "ofa"
                                                          THEN /\ ofaStartingMode(self[2])
                                                               /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].ofa = Failed, 
                                                                         "Failure of assertion at line 809, column 9 of macro called at line 1649, column 42.")
                                                          ...
                                                          ELSE /\ IF recoveredElem'[self] = "installer"
                                                                     THEN /\ installerInStartingMode(self[2])
                                                                          /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].installer = Failed, 
                                                                                    "Failure of assertion at line 860, column 9 of macro called at line 1650, column 48.")
                                                                          /\ switchStatus' = [switchStatus EXCEPT ![self[2]].installer = NotFailed]
                                                                          /\ IF switchStatus'[self[2]].nicAsic = NotFailed /\ switchStatus'[self[2]].ofa = NotFailed
                                                                                THEN /\ Assert(switchStatus'[self[2]].installer = NotFailed, 
                                                                                               "Failure of assertion at line 863, column 13 of macro called at line 1650, column 48.")
                                                                                     /\ auxEventCounter' = auxEventCounter + 1
                                                                                     /\ switchGeneratedEventSet' = [switchGeneratedEventSet EXCEPT ![self[2]] = switchGeneratedEventSet[self[2]] \cup {auxEventCounter'}]
                                                                                     /\ controlMsgCounter' = [controlMsgCounter EXCEPT ![self[2]] = controlMsgCounter[self[2]] + 1]
                                                                                     /\ statusResolveMsg' = [statusResolveMsg EXCEPT ![self] = [type |-> KEEP_ALIVE,
                                                                                ...
                                                                                ELSE /\ TRUE
                                                                                     /\ UNCHANGED << controlMsgCounter, 
                                                                                                     ...
                                                                                                     statusResolveMsg >>
                                                                     ELSE /\ Assert(FALSE, 
                                                                                    "Failure of assertion at line 1651, column 14.")
...
ghostProc(self) == /\ pc[self] = "ghostProc"
                   ...
                   /\ Assert(\/ switchLock[2] = self[2]
                             \/ switchLock[2] = NO_LOCK, 
                             "Failure of assertion at line 1009, column 9 of macro called at line 1758, column 9.")
                   /\ switchLock' = <<NO_LOCK, NO_LOCK>>
                   ...
                   /\ UNCHANGED << controllerLocalLock, controllerGlobalLock, 
                                   ...
                                   receivedEventsCopy, latestRecoveryEvent, 
...
sendIRQueueModNotification(self) == /\ pc[self] = "sendIRQueueModNotification"
                                    /\ IF subscriberOfcSet[self] # {}
                                          THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                               ...
                                               /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                               /\ controllerGlobalLock' = self
                                               /\ ofcID' = [ofcID EXCEPT ![self] = CHOOSE x \in subscriberOfcSet[self]: TRUE]
...
NibEventHandlerProc(self) == /\ pc[self] = "NibEventHandlerProc"
                             /\ IF ~shouldNIBEventHandlerTerminate(self[1])
                                   THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                           \/ controllerGlobalLock[1] = self[1]
                                        /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                        /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                        ...
                                        /\ Assert(event_'[self].type \in {INSTALL_FLOW}, 
                                                  "Failure of assertion at line 1893, column 13.")
                                        /\ IF event_'[self].type = INSTALL_FLOW
                                              THEN /\ IF masterState[self[1]] = ROLE_MASTER
                                                         THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                 \/ controllerGlobalLock[1] = self[1]
                                                              /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                              /\ controllerGlobalLock' = self
                                                              /\ workerLocalQueue' = [workerLocalQueue EXCEPT ![self[1]] = Append((workerLocalQueue[self[1]]), [item |-> event_'[self], id |-> -1, tag |-> NO_TAG])]
                                                              /\ UNCHANGED fetchedIRsBeforePassingToWorker
                                                         ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                 \/ controllerGlobalLock[1] = self[1]
                                                              /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                              /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                              /\ fetchedIRsBeforePassingToWorker' = [fetchedIRsBeforePassingToWorker EXCEPT ![self[1]] = Append(fetchedIRsBeforePassingToWorker[self[1]], event_'[self])]
                                   ...
                                   ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                        ...
                                        /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                        /\ controllerGlobalLock' = self
                                        /\ ofcModuleTerminationStatus' = [ofcModuleTerminationStatus EXCEPT ![self[1]][self[2]] = TERMINATE_DONE]
                                ...
                                /\ IF ofcFailoverStateNIB[self[1]] = FAILOVER_INIT
                                      THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                           ...
                                           /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                              \/ controllerLocalLock[self[1]] = self
                                           /\ controllerLocalLock' = [controllerLocalLock EXCEPT ![self[1]] = <<NO_LOCK, NO_LOCK>>]
                                           /\ isOfcEnabled' = [isOfcEnabled EXCEPT ![self[1]] = TRUE]
                                      ...
                                      ELSE /\ IF ofcFailoverStateNIB[self[1]] = FAILOVER_TERMINATE
                                                 THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                      ...
                                                      /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                      /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                      /\ subscribeList' = [subscribeList EXCEPT !.IRQueueNIB = subscribeList.IRQueueNIB \ {self[1]},
                                                 ...
                                                 ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                      ...
                                                      /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                      /\ controllerGlobalLock' = self
                                                      /\ currSW' = [currSW EXCEPT ![self] = CHOOSE x \in getSetSwitchInNonMasterRole(self[1]): TRUE]
...
WaitForSwitchUpdateRoleACK(self) == /\ pc[self] = "WaitForSwitchUpdateRoleACK"
                                    ...
                                    /\ IF existRecoveredSwitchWithSlaveRole(self[1])
                                          THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                               ...
                                               /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                               /\ controllerGlobalLock' = self
                                               /\ currSW' = [currSW EXCEPT ![self] = CHOOSE x \in setRecoveredSw[self[1]]: TRUE]
                                               /\ IF controllerRoleInSW[self[1]][currSW'[self]] = ROLE_SLAVE
                                                     ...
                                                     ELSE /\ TRUE
                                                          /\ UNCHANGED << workerLocalQueue, 
                                                                          roleUpdateStatus, 
                                                                          setScheduledRoleUpdates >>
                                               /\ setRecoveredSw' = [setRecoveredSw EXCEPT ![self[1]] = setRecoveredSw[self[1]] \ {currSW'[self]}]
                                               /\ pc' = [pc EXCEPT ![self] = "WaitForSwitchUpdateRoleACK"]
                                               /\ UNCHANGED << fetchedIRsBeforePassingToWorker, 
                                                               subscribeList, 
                                                               ofcFailoverStateNIB >>
...
ReconcileEventLogs(self) == /\ pc[self] = "ReconcileEventLogs"
                            /\ IF swSet[self] # {}
                                  ...
                                  ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                       ...
                                       /\ ofcModuleInitStatus' = [ofcModuleInitStatus EXCEPT ![self[1]][CONT_EVENT] = INIT_MASTER]
                                       /\ pc' = [pc EXCEPT ![self] = "ofcFailoverHandlerProc"]
                                       /\ UNCHANGED currSW
                            /\ UNCHANGED << switchLock, FirstInstall, 
                                            sw_fail_ordering_var, ContProcSet, 
                                            SwProcSet, controller2Switch, 
                                            switch2Controller, switchStatus, 
                                            installedIRs, installedBy, 
                                            swFailedNum, swNumEvent, 
                                            NicAsic2OfaBuff, Ofa2NicAsicBuff, 
...
ReconcileEventForSW(self) == /\ pc[self] = "ReconcileEventForSW"
                             /\ IF pulledNIBEventLog[self][currSW[self]] # <<>> /\ receivedEventsCopy[self][currSW[self]] # <<>>
                                   THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                        ...
                                        /\ Assert(event'[self].swID = currSW[self], 
                                                  "Failure of assertion at line 2031, column 33.")
                                        ...
                                        /\ IF event'[self].type = KEEP_ALIVE
                                              THEN /\ latestRecoveryEvent' = [latestRecoveryEvent EXCEPT ![self] = event'[self]]
                                              ELSE /\ TRUE
                                                   /\ UNCHANGED latestRecoveryEvent
                                        /\ pc' = [pc EXCEPT ![self] = "ReconcileEventForSW"]
                                   ...
                                   ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                        ...
                                        /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                           \/ controllerGlobalLock[1] = self[1]
                                        /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                        /\ IF receivedEventsCopy[self][currSW[self]] # <<>>
                                              THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                      \/ controllerGlobalLock[1] = self[1]
                                                   /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                   /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                   ...
                                                   /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                   /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                   /\ remainingEvents' = [remainingEvents EXCEPT ![self] = remainingEvents[self] \o receivedEventsCopy[self][currSW[self]]]
                                                   /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][currSW[self]] = NIBSwSuspensionStatus[currSW[self]]]
                                                   /\ UNCHANGED << eventSkipList, 
                                                                   latestRecoveryEvent >>
                                              ELSE /\ IF pulledNIBEventLog[self][currSW[self]] # <<>>
                                                         THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                 \/ controllerGlobalLock[1] = self[1]
                                                              /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                              /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                              ...
                                                              /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                              /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                              /\ eventSkipList' = [eventSkipList EXCEPT ![self[1]][currSW[self]] = eventSkipList[self[1]][currSW[self]] \o pulledNIBEventLog[self][currSW[self]]]
                                                              /\ UNCHANGED << ofcSwSuspensionStatus, 
                                                                              remainingEvents, 
                                                                              latestRecoveryEvent >>
                                                         ELSE /\ IF shouldPerformDataPlaneReconciliation(currSW[self], self[1], remainingEvents[self]) /\ latestRecoveryEvent[self].type # 0
                                                                    THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                            \/ controllerGlobalLock[1] = self[1]
                                                                         /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                         /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                                         ...
                                                                         /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                         /\ controllerGlobalLock' = self
                                                                         /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][currSW[self]] = SW_SUSPEND]
                                                                         /\ eventSkipList' = [eventSkipList EXCEPT ![self[1]][currSW[self]] = Append(eventSkipList[self[1]][currSW[self]], latestRecoveryEvent[self])]
                                                                         /\ remainingEvents' = [remainingEvents EXCEPT ![self] = Append(remainingEvents[self], latestRecoveryEvent[self])]
                                                                         /\ latestRecoveryEvent' = [latestRecoveryEvent EXCEPT ![self] = [type |-> 0]]
                                                                    ELSE /\ TRUE
                                                                         /\ UNCHANGED << controllerLocalLock, 
                                                                                         controllerGlobalLock, 
                                                                                         ofcSwSuspensionStatus, 
                             ...
                             /\ UNCHANGED << switchLock, FirstInstall, 
                                             ...
                                             idThreadWorkingOnIR, 
                                             workerThreadRanking, 
                                             workerLocalQueue, processedEvents, 
                                             localEventLog, controllerRoleInSW, 
                                             nibEventQueue, roleUpdateStatus, 
                                             flowStatReqStatus, isOfcEnabled, 
                                             setScheduledRoleUpdates, 
...
WaitForWorkersTermination(self) == /\ pc[self] = "WaitForWorkersTermination"
                                   ...
                                   /\ ofcModuleTerminationStatus' = [ofcModuleTerminationStatus EXCEPT ![self[1]] = [y \in {CONT_MONITOR, NIB_EVENT_HANDLER} |-> TERMINATE_INIT]
                                                                                                                                         @@ ofcModuleTerminationStatus[self[1]]]
                                                ...
                                                /\ IF (stepOfFailure_c'[self] = 2)
                                                      ...
                                                      ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                           ...
                                                           /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                              \/ controllerLocalLock[self[1]] = self
                                                           /\ controllerLocalLock' = [controllerLocalLock EXCEPT ![self[1]] = <<NO_LOCK, NO_LOCK>>]
                                                           /\ IF idThreadWorkingOnIR[self[1]][entryIndex'[self]] = IR_UNLOCK
                                                                 THEN /\ threadWithLowerIDGetsTheLock(self[1], self, nextToSent'[self], workerLocalQueue'[self[1]])
                                                                      ...
                                                                      /\ IF (stepOfFailure_c'[self] = 3)
                                                                            ...
                                                                            ELSE /\ IF (workerCanSendTheIR(self[1], nextToSent'[self]))
                                                                                       THEN /\ Assert(nextToSent'[self].type \in {INSTALL_FLOW, ROLE_REQ, FLOW_STAT_REQ}, 
                                                                                                      "Failure of assertion at line 2170, column 29.")
                                                                                            /\ IF nextToSent'[self].type = INSTALL_FLOW
                                                                                                  THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                                          \/ controllerGlobalLock[1] = self[1]
                                                                                                       /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                                                       /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                                                                       /\ IRStatus' = [IRStatus EXCEPT ![nextToSent'[self].IR] = IR_SENT]
                                                                                                  ...
                                                                                                  ELSE /\ IF nextToSent'[self].type = ROLE_REQ
                                                                                                             ...
                                                                                                             ELSE /\ IF nextToSent'[self].type = FLOW_STAT_REQ
                                                                                                                        THEN /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][nextToSent'[self].to] = IR_SENT]
                                                                                                                        ELSE /\ TRUE
                                                                                                                             /\ UNCHANGED flowStatReqStatus
                                                                                                                  /\ UNCHANGED roleUpdateStatus
                                                                                                       /\ UNCHANGED << controllerGlobalLock, 
                                                         ...
                                                         /\ IF (stepOfFailure_c'[self] # 1)
                                                               THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                    ...
                                                                    /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                                       \/ controllerLocalLock[self[1]] = self
                                                                    /\ controllerLocalLock' = [controllerLocalLock EXCEPT ![self[1]] = <<NO_LOCK, NO_LOCK>>]
                                                                    /\ IF idThreadWorkingOnIR[self[1]][entryIndex[self]] = self[2]
                                                                    ...
                                                                    /\ IF (stepOfFailure_c'[self] # 2)
                                                                          THEN /\ IF nextToSent[self].type = INSTALL_FLOW
                                                                                     THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                          ...
                                                                                          /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                                          /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                                                          /\ SetScheduledIRs' = [SetScheduledIRs EXCEPT ![nextToSent[self].to] = SetScheduledIRs[nextToSent[self].to]\{nextToSent[self].IR}]
                                                                                     ELSE /\ TRUE
                                                                                          /\ UNCHANGED << controllerGlobalLock, 
                                                                                                          SetScheduledIRs >>
                                                                               /\ IF (stepOfFailure_c'[self] # 3)
                                   ...
                                   /\ IF (stepOfFailure_c'[self] # 1)
                                         THEN /\ IF nextToSent[self].type = INSTALL_FLOW
                                                    ...
                                                    ELSE /\ IF nextToSent[self].type = ROLE_REQ
                                                               ...
                                                               ELSE /\ IF nextToSent[self].type = FLOW_STAT_REQ
                                                                          THEN /\ controller2Switch' = [controller2Switch EXCEPT ![nextToSent[self].to] = Append(controller2Switch[nextToSent[self].to], [type |-> FLOW_STAT_REQ,
                                                                                                                                                                                                          from |-> self[1],
                                                                                                                                                                                                          to |-> nextToSent[self].to,
                                                                                                                                                                                                          IR |-> nextToSent[self].IR])]
                                                                          ELSE /\ TRUE
                                             ...
                                             /\ IF (controllerStateNIB[self].type = STATUS_LOCKING)
                                                   THEN /\ IF (controllerStateNIB[self].next.type = INSTALL_FLOW)
                                                              ...
                                                              ELSE /\ IF (controllerStateNIB[self].next.type = FLOW_STAT_REQ)
                                                                         THEN /\ IF (flowStatReqStatus[self[1]][controllerStateNIB[self].next.to] = IR_SENT)
                                                                                    THEN /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][controllerStateNIB[self].next.to] = IR_NONE]
                                                                                    ELSE /\ TRUE
                                                                                         /\ UNCHANGED flowStatReqStatus
                                                                              /\ UNCHANGED roleUpdateStatus
...
ControllerEventHandlerProc(self) == /\ pc[self] = "ControllerEventHandlerProc"
                                    /\ IF ~shouldEventHandlerTerminate(self[1])
                                          THEN /\ isOfcEnabled[self[1]]
                                               ...
                                               /\ Assert(ofcModuleInitStatus[self[1]][self[2]] \in {INIT_NONE, INIT_MASTER, INIT_PROCESS}, 
                                                         "Failure of assertion at line 2355, column 10.")
                                               /\ ofcModuleInitStatus[self[1]][self[2]] \in {INIT_PROCESS, INIT_MASTER}
                                               /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                  \/ controllerGlobalLock[1] = self[1]
                                               /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                               /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                               ...
                                               /\ IF eventSkipList[self[1]][monitoringEvent'[self].swID] # <<>>
                                                     THEN /\ IF areEventsEquivalent(monitoringEvent'[self], Head(eventSkipList[self[1]][monitoringEvent'[self].swID]))
                                                                     ...
                                                                     /\ IF shouldSuspendSw(monitoringEvent'[self]) /\ ofcSwSuspensionStatus[self[1]][monitoringEvent'[self].swID] = SW_RUN
                                                                           ...
                                                                           ELSE /\ IF canfreeSuspendedSw(monitoringEvent'[self]) /\ ofcSwSuspensionStatus[self[1]][monitoringEvent'[self].swID] = SW_SUSPEND
                                                                                      THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                           ...
                                                                                           /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                                           /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                                                           /\ IF shouldSendFlowStatReqForAllEntriesOnReconciliation(monitoringEvent'[self], self[1])
                                                                                                 THEN /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][(monitoringEvent'[self].swID)] = SW_RECONCILE]
                                                                                                      /\ IF masterState[self[1]] = ROLE_MASTER
                                                                                                            THEN /\ NIBSwSuspensionStatus' = [NIBSwSuspensionStatus EXCEPT ![(monitoringEvent'[self].swID)] = SW_RECONCILE]
                                                                                                            ELSE /\ TRUE
                                                                                                                 /\ UNCHANGED NIBSwSuspensionStatus
                                                                                                      /\ workerLocalQueue' = [workerLocalQueue EXCEPT ![self[1]] = Append((workerLocalQueue[self[1]]), [item |-> ([type |-> FLOW_STAT_REQ,
                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                   IR |-> ALL_IR]), id |-> -1, tag |-> NO_TAG])]
                                                                                                 ELSE /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][monitoringEvent'[self].swID] = SW_RUN]
                                                                                                      /\ UNCHANGED << workerLocalQueue, 
                                                                                                                      NIBSwSuspensionStatus >>
                                                                                      ELSE /\ TRUE
                                                                                           /\ UNCHANGED << controllerGlobalLock, 
                                                          ...
                                                          /\ UNCHANGED << controllerSubmoduleFailNum, 
                                                                          ...
                                                                          stepOfFailure_co, 
                                                                          isEventProcessed >>
                                                     ELSE /\ IF shouldSuspendSw(monitoringEvent'[self]) /\ ofcSwSuspensionStatus[self[1]][monitoringEvent'[self].swID] = SW_RUN
                                                                     ...
                                                                     /\ IF (controllerSubmoduleFailStat'[self] = NotFailed)
                                                                           THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                ...
                                                                                /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                                                   \/ controllerLocalLock[self[1]] = self
                                                                                /\ controllerLocalLock' = [controllerLocalLock EXCEPT ![self[1]] = <<NO_LOCK, NO_LOCK>>]
                                                                                /\ IF (ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER)
                                                                                      THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                              \/ controllerGlobalLock[1] = self[1]
                                                                                           /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                                           /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                                                           /\ IF (masterState[self[1]] = ROLE_MASTER)
                                                                                                 THEN /\ isEventProcessed' = [isEventProcessed EXCEPT ![self] = 1]
                                                                                                      /\ NIBSwSuspensionStatus' = [NIBSwSuspensionStatus EXCEPT ![monitoringEvent'[self].swID] = SW_SUSPEND]
                                                                                                 ELSE /\ TRUE
                                                                                                      /\ UNCHANGED << NIBSwSuspensionStatus, 
                                                                                                                      isEventProcessed >>
                                                                                      ELSE /\ TRUE
                                                                                           ...
                                                                                           /\ IF (stepOfFailure_co'[self] = 2)
                                                                                                 THEN /\ controllerSubmoduleFailStat' = [controllerSubmoduleFailStat EXCEPT ![self] = Failed]
                                                                                                      ...
                                                                                                      /\ UNCHANGED << controllerLocalLock, 
                                                                                                                      ...
                                                                                                                      NIBSwSuspensionStatus, 
                                                                                                                      isEventProcessed >>
                                                                                                 ELSE /\ setRecoveredSw' = [setRecoveredSw EXCEPT ![self[1]] = setRecoveredSw[self[1]] \cup {monitoringEvent'[self].swID}]
                                                                                                      /\ IF (ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER)
                                                                                                            THEN /\ isEventProcessed' = [isEventProcessed EXCEPT ![self] = 1]
                                                                                                                 /\ IF (stepOfFailure_co'[self] = 3)
                                                                                                                       ...
                                                                                                                       ELSE /\ IF ~existsMonitoringEventHigherNum(monitoringEvent'[self], self[1])
                                                                                                                                  THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                                                                       ...
                                                                                                                                       /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                                                                                       /\ controllerGlobalLock' = self
                                                                                                                                       /\ IF controllerRoleInSW[self[1]][monitoringEvent'[self].swID] # ROLE_MASTER
                                                                                                                                  ...
                                                                                                                                  ELSE /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                                                                       ...
                                                                                                                                       /\ IF (masterState[self[1]] = ROLE_MASTER)
                                                                                                                                             THEN /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                                                                                                                  ...
                                                                                                                                                  /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                                                                                                                                  /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                                                                                                                  /\ NIBSwSuspensionStatus' = [NIBSwSuspensionStatus EXCEPT ![monitoringEvent'[self].swID] = SW_RUN]
                                                                                                                                             ELSE /\ TRUE
                                                                                                                                                  /\ UNCHANGED << controllerGlobalLock, 
                                                                                                                                                                  NIBSwSuspensionStatus >>
                                                                                                                                       /\ pc' = [pc EXCEPT ![self] = "ControllerEvenHanlderRemoveEventFromQueue"]
...
ScheduleFlowStatReq(self) == /\ pc[self] = "ScheduleFlowStatReq"
                             ...
                             /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                \/ controllerLocalLock[self[1]] = self
                             /\ controllerLocalLock' = [controllerLocalLock EXCEPT ![self[1]] = <<NO_LOCK, NO_LOCK>>]
                             /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][(monitoringEvent[self].swID)] = SW_RECONCILE]
                             /\ IF masterState[self[1]] = ROLE_MASTER
                                   THEN /\ NIBSwSuspensionStatus' = [NIBSwSuspensionStatus EXCEPT ![(monitoringEvent[self].swID)] = SW_RECONCILE]
                                   ELSE /\ TRUE
                                        /\ UNCHANGED NIBSwSuspensionStatus
                                       ...
                                       /\ IF ~shouldMonitoringServerTerminate(self[1])
                                             THEN /\ isOfcEnabled[self[1]]
                                                  ...
                                                  /\ Assert(\/ /\ msg'[self].type = INSTALLED_SUCCESSFULLY
                                                            ...
                                                            "Failure of assertion at line 2605, column 9.")
                                                  /\ IF (controllerSubmoduleFailNum[self[1]] < getMaxNumSubModuleFailure(self[1]))
                                                        THEN /\ \E num \in 0..2:
                                                             ...
                                                             /\ IF (stepOfFailure'[self] # 1)
                                                                   ...
                                                                   ELSE /\ TRUE
                                                                        /\ UNCHANGED << FirstInstall, 
                                                                                        IRStatus >>
                                                             /\ pc' = [pc EXCEPT ![self] = "MonitoringServerLazyRemoveFromQueue"]
                                                             /\ UNCHANGED << ofcSwSuspensionStatus, 
                                                                             controllerRoleInSW, 
                                                                             roleUpdateStatus, 
                                                                             flowStatReqStatus, 
                                                        ...
                                                        ELSE /\ IF msg'[self].type = FLOW_STAT_REPLY
                                                                   THEN /\ \/ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
                                                                        ...
                                                                        /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                                                                           \/ controllerLocalLock[self[1]] = self
                                                                        /\ controllerLocalLock' = [controllerLocalLock EXCEPT ![self[1]] = <<NO_LOCK, NO_LOCK>>]
                                                                        /\ Assert(msg'[self].status \in {ALL_IR, NO_ENTRY, ENTRY_FOUND}, 
                                                                                  "Failure of assertion at line 2627, column 13.")
                                                                        /\ Assert(ofcSwSuspensionStatus[self[1]][msg'[self].from] \in {SW_RECONCILE}, 
                                                                                  "Failure of assertion at line 2628, column 13.")
                                                                        /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][msg'[self].from] = IR_DONE]
                                                                        /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][msg'[self].from] = SW_RUN]
                                                                        /\ IF masterState[self[1]] = ROLE_MASTER
                                                                              THEN /\ NIBSwSuspensionStatus' = [NIBSwSuspensionStatus EXCEPT ![msg'[self].from] = SW_RUN]
                                                                              ELSE /\ TRUE
                                                                                   /\ UNCHANGED NIBSwSuspensionStatus
                                                                        /\ IF msg'[self].status = ALL_IR
                                                                              THEN /\ setIRs' = [setIRs EXCEPT ![self] = getSetIRsForSwitch(msg'[self].from)]
                                                                                   /\ pc' = [pc EXCEPT ![self] = "changeStatusOfIRs"]
                                                                                   /\ UNCHANGED IRStatus
                                                                              ELSE /\ IF msg'[self].status = ENTRY_FOUND
                                                                                         THEN /\ IRStatus' = [IRStatus EXCEPT ![msg'[self].IR] = IR_DONE]
                                                                                         ELSE /\ IRStatus' = [IRStatus EXCEPT ![msg'[self].IR] = IR_NONE]
                                                                                   /\ pc' = [pc EXCEPT ![self] = "MonitoringServerLazyRemoveFromQueue"]
                                                                                   /\ UNCHANGED setIRs
...
MonitoringServerLazyRemoveFromQueue(self) == /\ pc[self] = "MonitoringServerLazyRemoveFromQueue"
                                             /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                                                \/ controllerGlobalLock[1] = self[1]
                                             /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                             /\ controllerGlobalLock' = <<NO_LOCK, NO_LOCK>>
...
changeStatusOfIRs(self) == /\ pc[self] = "changeStatusOfIRs"
                           /\ \/ controllerGlobalLock = <<NO_LOCK, NO_LOCK>>
                              \/ controllerGlobalLock[1] = self[1]
                           /\ switchLock = <<NO_LOCK, NO_LOCK>>
                           /\ \/ controllerLocalLock[self[1]] = <<NO_LOCK, NO_LOCK>>
                           ...
                           /\ IF currIR'[self] \in msg[self].IR
                                 THEN /\ IRStatus' = [IRStatus EXCEPT ![currIR'[self]] = IR_DONE]
                                 ELSE /\ IRStatus' = [IRStatus EXCEPT ![currIR'[self]] = IR_NONE]
                           /\ IF setIRs'[self] = {}
                                 THEN /\ pc' = [pc EXCEPT ![self] = "MonitoringServerLazyRemoveFromQueue"]
                                 ELSE /\ pc' = [pc EXCEPT ![self] = "changeStatusOfIRs"]
                           /\ UNCHANGED << switchLock, FirstInstall, 
                                           sw_fail_ordering_var, ContProcSet, 
                                           SwProcSet, swSeqChangedStatus, 
                                           controller2Switch, 
...
controllerMonitoringServer(self) == ControllerMonitorCheckIfMastr(self)
                                       \/ MonitoringServerLazyRemoveFromQueue(self)
                                       \/ changeStatusOfIRs(self)
...
ControllerWatchDogProc(self) == /\ pc[self] = "ControllerWatchDogProc"
                                ...
                                /\ \E module \in controllerFailedModules'[self]:
                                     /\ Assert(controllerSubmoduleFailStat[module] = Failed, 
                                               "Failure of assertion at line 2719, column 13.")
                                     /\ controllerLocalLock' = module
                                     /\ controllerGlobalLock' = module
...
\* END TRANSLATION - the hash of the generated TLA code (remove to silence divergence warnings): TLA-81db3b9af0ac9341b3d3b6cc653490b2
...
\* Last modified Tue Mar 16 06:18:21 UTC 2021 by root
