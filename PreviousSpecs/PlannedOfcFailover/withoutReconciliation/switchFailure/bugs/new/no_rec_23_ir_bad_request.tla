            ...
            if (stepOfFailure = 2) then
                ...
                if idThreadWorkingOnIR[self[1]][entryIndex] = IR_UNLOCK then
                    ...
                    if (stepOfFailure = 3) then
                        ...
                        await ofcSwSuspensionStatus[self[1]][nextToSent.to] # SW_ROLE_RECONCILE;
                        ...
                        if (stepOfFailure = 2) then
                            ...
                            ControllerFreeSuspendedSW:
                               ...
                               if (controllerRoleInSW[self[1]][monitoringEvent.swID] = ROLE_SLAVE /\ ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER) then
                                  ofcSwSuspensionStatus[self[1]][monitoringEvent.swID] := SW_ROLE_RECONCILE;
                               else
                                  ofcSwSuspensionStatus[self[1]][monitoringEvent.swID] := SW_RUN;
                               end if;
                               if (ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER) then  
    ...
    while ~shouldMonitoringServerTerminate(self[1]) do
        ...
        if msg.type = INSTALLED_SUCCESSFULLY then
                ...
                if (stepOfFailure # 1) then
                    ...
                    if ofcSwSuspensionStatus[self[1]][msg.from] = SW_ROLE_RECONCILE then
                                                                      ...
                                                                      /\ IF (stepOfFailure_c'[self] = 3)
                                                                            THEN /\ controllerSubmoduleFailStat' = [controllerSubmoduleFailStat EXCEPT ![self] = Failed]
                                                                                 ...
                                                                                 /\ UNCHANGED << roleUpdateStatus, 
                                                                                                 IRStatus >>
                                                                            ELSE /\ ofcSwSuspensionStatus[self[1]][nextToSent'[self].to] # SW_ROLE_RECONCILE
...
ControllerFreeSuspendedSW(self) == /\ pc[self] = "ControllerFreeSuspendedSW"
                                   ...
                                   /\ IF (controllerRoleInSW[self[1]][monitoringEvent[self].swID] = ROLE_SLAVE /\ ofcModuleInitStatus[self[1]][self[2]] = INIT_MASTER)
                                         THEN /\ ofcSwSuspensionStatus' = [ofcSwSuspensionStatus EXCEPT ![self[1]][monitoringEvent[self].swID] = SW_ROLE_RECONCILE]
                                                        ...
                                                        ELSE /\ IF msg'[self].type = ROLE_REPLY
                                                                   THEN /\ IF (stepOfFailure'[self] # 1)
                                                                              THEN /\ IF roleUpdateStatus[self[1]][msg'[self].from].roletype = msg'[self].roletype
                                                                                   ...
                                                                                   /\ IF ofcSwSuspensionStatus[self[1]][msg'[self].from] = SW_ROLE_RECONCILE
                                                                   ...
                                                                   ELSE /\ IF msg'[self].type = BAD_REQUEST
                                                                              THEN /\ TRUE
                                                                              ELSE /\ Assert(FALSE, 
                                                                                             "Failure of assertion at line 2474, column 13.")
                                                                        /\ UNCHANGED << ofcSwSuspensionStatus, 
                                                                                        controllerRoleInSW, 
                                                                                        roleUpdateStatus >>
                                                             /\ UNCHANGED << FirstInstall, 
...
ControllerWatchDogProc(self) == /\ pc[self] = "ControllerWatchDogProc"
                                ...
                                /\ \E module \in controllerFailedModules'[self]:
                                     /\ Assert(controllerSubmoduleFailStat[module] = Failed, 
                                               "Failure of assertion at line 2513, column 13.")
                                     /\ controllerLocalLock' = module
                                     /\ controllerGlobalLock' = module
...
\* Last modified Tue Jan 16 10:42:47 PST 2024 by pooria
\* Last modified Tue Jan 16 07:42:25 PST 2024 by root
