---------------- MODULE PlannedOfcFailoverWithReconciliation ----------------
...
CONSTANTS OFC_FAILOVER,
          ...
          TERMINATE_INIT,
          TERMINATE_DONE, 
          TERMINATE_UNGRACEFULLY,
          FAILOVER_TERMINATE_DONE,
          FAILOVER_NONE,
          ROLE_MASTER_INIT,
          ROLE_SLAVE_INIT 
          \* MASTER_INIT simplies the process of notification to the new OFC
          \* about its role status change in NIB. In the first round, 
          \* the new OFC's role changes to MASTER_INIT. After, reconciliation
          \* NIB event handler changes this to ROLE_MASTER.
...
(************************* variables ***************************************)
    variables (*************** Some Auxiliary Vars ***********************)
              ...
              \* local variable of OFC used for optimization 
              nextToSent = [x \in {ofc0, ofc1} |-> [to |-> 0]],
              \************* NIB Event Handler ************
              ...
              flowStatReqStatus = [x \in {ofc0, ofc1} |-> [y \in 1..MAX_NUM_IRs |-> IR_NONE]],
              ...
              \* after performing the reconciliation, these set is used to keep track of IRs that
              \* the OFC should re-send
              setIRToReschedule = [x \in {ofc0, ofc1} |-> {}],
    ...
    define
        ...
        switchShouldReleaseLock(sw) == /\ pc[<<NIC_ASIC_IN, sw>>] = "SwitchRcvPacket"
                                       /\ pc[<<OFA_IN, sw>>] = "SwitchOfaProcIn"
                                       /\ pc[<<OFA_OUT, sw>>] = "SwitchOfaProcOut"
                                       /\ pc[<<INSTALLER, sw>>] = "SwitchInstallerProc"
                                       /\ NicAsic2OfaBuff[sw] = <<>>
                                       /\ Ofa2InstallerBuff[sw] = <<>>
                                       /\ Ofa2NicAsicBuff[sw] = <<>>
                                       /\ Installer2OfaBuff[sw] = <<>>
                                       /\ controller2Switch[sw] = <<>> 
        noPacketInFly(sw) == ~\E x \in ({ofc0, ofc1} \X CONTROLLER_THREAD_POOL): /\ pc[x] = "ControllerWorkerIRTransmitted"
                                                                                 /\ nextToSent[x[1]].to = sw                             
        \***************************** OFA **********************************
        ...
        existMatchingEntryTCAM(swID, irID) == \E x \in rangeSeq(TCAM[swID]): x = irID
        ...
        isProcTerminated(CID, TID) == ofcModuleTerminationStatus[CID][TID] = TERMINATE_UNGRACEFULLY
        
        \********************** NIB Event Handler ****************************
        allSwitchInEqualRole(CID) == \A x \in SW: controllerRoleInSW[CID][x] = ROLE_EQUAL
        allSwitchInMasterRole(CID) == \A x \in SW: controllerRoleInSW[CID][x] = ROLE_MASTER
        ...
        allWorkersTerminated(CID) == \A x \in CONTROLLER_THREAD_POOL: 
                                                        ofcModuleTerminationStatus[CID][x] = TERMINATE_DONE
        getSetIrReconciliation == {x \in 1..MAX_NUM_IRs: IRStatus[x] = IR_SENT}
        ...
        workerCanSendTheIR(CID, buffEntry) == /\ ~isSwitchSuspended(buffEntry.to)
                                              /\ \/ /\ buffEntry.type = ROLE_REQ
                                                    /\ roleUpdateStatus[CID][buffEntry.to] = IR_NONE
                                                 \/ /\ buffEntry.type = INSTALL_FLOW
                                                    /\ IRStatus[buffEntry.IR] = IR_NONE
                                                 \/ /\ buffEntry.type = FLOW_STAT_REQ
                                                    /\ flowStatReqStatus[CID][buffEntry.IR] = IR_NONE
        workerShouldFastRecovery(CID, buffEntry) == \/ /\ buffEntry.type = ROLE_REQ
                                                       /\ roleUpdateStatus[CID][buffEntry.to] = IR_NONE
                                                    \/ /\ buffEntry.type = INSTALL_FLOW
                                                       /\ IRStatus[buffEntry.IR] = IR_NONE
    ...
    macro sendFlowStatReplyNotFound(controllerID, irID)
    begin
        Ofa2NicAsicBuff[self[2]] := Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                      from |-> self[2],
                                                                      to |-> controllerID,
                                                                      status |-> NO_ENTRY,
                                                                      IR |-> irID]);
    ...
    macro sendFlowStatReplyEntryFound(controllerID, irID)
    begin
        Ofa2NicAsicBuff[self[2]] := Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                      from |-> self[2],
                                                                      to |-> controllerID,
    ...
    while TRUE do
        ...
        if ingressPkt.type = INSTALL_FLOW then 
            ...
            assert ingressPkt.type \in 1..MAX_NUM_IRs;
            if existMatchingEntryTCAM(self[2], ingressPkt.IR) then
                sendFlowStatReplyEntryFound(ingressPkt.from, ingressPkt.IR);
            else
                sendFlowStatReplyNotFound(ingressPkt.from, ingressPkt.IR);
            end if;
        end if;
        releaseLock(self);
    end while;
    ...
    while TRUE do
        ...
        await Len(controller2Switch[self[2]]) > 0;
        await noPacketInFly(self[2]);
        ingressIR := Head(controller2Switch[self[2]]);
        assert ingressIR.type \in {ROLE_REQ, INSTALL_FLOW, FLOW_STAT_REQ};
    ...
    while TRUE do
        ...
        acquireLock();
        assert egressMsg.type \in {INSTALLED_SUCCESSFULLY, ROLE_REPLY, BAD_REQUEST, FLOW_STAT_REPLY};
        ...
        SwitchNicAsicSendOutMsg:
            if swCanReceivePackets(self[2]) then
                ...
                if switchShouldReleaseLock(self[2]) then
                    releaseLock(self);
                end if;
            else
                egressMsg := [type |-> 0];
    ...
    while TRUE do
        ...
        assert \/ /\ ofaInMsg.type = ROLE_REQ
               ...
               \/ /\ ofaInMsg.type \in {INSTALL_FLOW, FLOW_STAT_REQ}
        ...
        SwitchOfaProcessPacket:
           if swOFACanProcessIRs(self[2]) then
                ...
                if ofaInMsg.type = INSTALL_FLOW then
                    ...
                    \* 2.4. Switch upon receiving FLOW_STAT_REQ message must check whether there is a matching 
                    ...
                    assert ofaInMsg.IR \in 1..MAX_NUM_IRs;
                    acquireLock();
                    if existMatchingEntryTCAM(self[2], ofaInMsg.IR) then
                        sendFlowStatReplyEntryFound(ofaInMsg.from, ofaInMsg.IR);
                    else
                        sendFlowStatReplyNotFound(ofaInMsg.from, ofaInMsg.IR);
                    end if; 
                end if;
           else
                ofaInMsg := [type |-> 0];
...
\*        releaseLock(self);
    ...
    variables event=[type |-> 0], irSet = {}, currIR = 0, currSW = 0, swSet = {},
              IRQueueEntries = [type |-> 0], entry = [type |-> 0];
    begin
    NibEventHandlerProc:
        while TRUE do
            await \/ masterState[self[1]] = ROLE_MASTER_INIT
                  \/ masterState[self[1]] = ROLE_SLAVE_INIT
                  \/ setIRToReschedule[self[1]] # {}
                  \/ /\ masterState[self[1]] = ROLE_MASTER
                     /\ nibEventQueue[self[1]] # <<>>;
            
            \* if NIB event handler receives notification about change in its role to slave;
            \* 1. unsubscrine from NIB IR Queue
            \* 2. terminate other modules in OFC
            if masterState[self[1]] = ROLE_SLAVE_INIT then 
              
                subscribeList.IRQueueNIB := subscribeList.IRQueueNIB \ {self[1]}; 
                ofcModuleTerminationStatus[self[1]] := 
                                [y \in (CONTROLLER_THREAD_POOL \cup {CONT_MONITOR, CONT_EVENT}) |-> TERMINATE_UNGRACEFULLY];
                ofcFailoverStateNIB[self[1]] := FAILOVER_TERMINATE_DONE;
                masterState[self[1]] := ROLE_SLAVE;
            
            \* if NIB event handler receives notification about change in its role to master;
            \* 1. update its role to ROLE_MASTER in all the switches
            \* 2. pull the IRs with flag IR_SENT and schedule the corresponding flow stat req messages
            \* for reconciliation
            \* 3. reconcile worker local queue with IR Queue in NIB and subscribe to IR Queue in NIB 
            elsif masterState[self[1]] = ROLE_MASTER_INIT then 
                \* Step 1. update role to ROLE_MASTER in all the switches
            
                \* controllerAcquireLock(self);
                isOfcEnabled[self[1]] := TRUE; \* This is for optimization
                swSet := getSetSwitchInSlaveRole(self[1]) \cup getSetSwitchInEqualRole(self[1]);
            
                ScheduleRoleUpdateMaster:
                    while swSet # {} do
                        currSW := CHOOSE x \in swSet: TRUE;
                        modifiedEnqueue(workerLocalQueue[self[1]], 
                                           [type |-> ROLE_REQ, roletype |-> ROLE_MASTER, to |-> currSW]);
                        swSet := swSet \ {currSW};  
                    end while;
                    \* Step 2: (1) wait for receiving confirmation from all switches regarding 
                    \* role update. (2) schedule the flow stat requests
                    await allSwitchInMasterRole(self[1]);
                    irSet := getSetIrReconciliation;
                    
                ScheduleFlowStatReq: \* @Todo(Pooria): should consider failure case
                    while irSet # {} do 
                        currIR := CHOOSE x \in irSet: TRUE;
                        IRStatus[currIR] := IR_RECONCILE;
                        modifiedEnqueue(workerLocalQueue[self[1]], [type |-> FLOW_STAT_REQ, 
                                                                    to |-> IR2SW[currIR], 
                                                                    IR |-> currIR]);
                        irSet := irSet \ {currIR}
                    end while;             
                
                \* Step 3. reconcile with IRQueue and subscribe to IRQueueNIB
                subscribeList.IRQueueNIB := subscribeList.IRQueueNIB \cup {self[1]};
                IRQueueEntries := IRQueueNIB; 
                    
                QueryIRQueueNIB:
                    \* (1) Pull IRQueue and add to worker queue
                    \* (2) change your masterState to ROLE_MASTER in NIB
            ...
            \* if exists IR that has to be rescheduled, NIB event handler reschedules it.
            \* 1. add to scheduledset
            \* 2. change flag to IR_NONE to be treated same as a normal IR
            \* 3. add the IR to workers local queue. 
            elsif setIRToReschedule[self[1]] # {} then
            
                currIR := CHOOSE x \in setIRToReschedule[self[1]]: TRUE;
                assert IRStatus[currIR] = IR_RECONCILE; 
                SetScheduledIRs[IR2SW[currIR]] := SetScheduledIRs[IR2SW[currIR]] \cup {currIR}; 
                IRStatus[currIR] := IR_NONE;
                flowStatReqStatus[self[1]][currIR] := IR_NONE;
                modifiedEnqueue(workerLocalQueue[self[1]], [type |-> INSTALL_FLOW, 
                                                           to |-> IR2SW[currIR], 
                                                           IR |-> currIR]);
                setIRToReschedule[self[1]] := setIRToReschedule[self[1]] \ {currIR};
    ...
    variables entryIndex=-1, rowIndex=-1, rowRemove=-1, 
    ...
    while ~shouldWorkerTerminate(self[1], self[2]) do
        ...
        \* following checks whether the proc has been terminated or not.
        await ~isProcTerminated(self[1], self[2]);
        controllerWaitForLockFree();
        ...
        modifiedRead(workerLocalQueue[self[1]], nextToSent[self[1]], entryIndex);
        ...
        if (stepOfFailure = 1) then
            ...
            controllerStateNIB[self] := [type |-> STATUS_LOCKING, next |-> nextToSent[self[1]], index |-> entryIndex];
            if (stepOfFailure = 2) then
                ...
                if idThreadWorkingOnIR[self[1]][entryIndex] = IR_UNLOCK then
                    await threadWithLowerIDGetsTheLock(self[1], self, nextToSent[self[1]], workerLocalQueue[self[1]]); \* For Reducing Space
                    idThreadWorkingOnIR[self[1]][entryIndex] := self[2];
                else
                    ControllerThreadRemoveQueue1: 
                        await ~isProcTerminated(self[1], self[2]);
                        controllerWaitForLockFree();
                        modifiedRemove(workerLocalQueue[self[1]], nextToSent[self[1]]);
                        if nextToSent[self[1]].type = INSTALL_FLOW then
                            \* ony master OFC has write access to IR Queue
                            if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then  
                                removeItem(IRQueueNIB, nextToSent[self[1]]);
                            end if;
                        end if;
        ...
        ControllerThreadSendIR:
            ...
            if (controllerSubmoduleFailStat[self] = NotFailed) then
                ...
                if workerCanSendTheIR(self[1], nextToSent[self[1]]) then
                    \**** Step 1.1: change the status of the IR to IR_SENT before actually sending
                    \**** the IR (Update-before-Action)
                    assert nextToSent[self[1]].type \in {INSTALL_FLOW, ROLE_REQ, FLOW_STAT_REQ};
                    if nextToSent[self[1]].type = INSTALL_FLOW then
                        \* only Master OFC has write access to IRStatus
                        if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then 
                            IRStatus[nextToSent[self[1]].IR] := IR_SENT;
                        end if;
                    elsif nextToSent[self[1]].type = ROLE_REQ then
                        roleUpdateStatus[self[1]][nextToSent[self[1]].to] := IR_SENT;
                    elsif nextToSent[self[1]].type = FLOW_STAT_REQ then
                        flowStatReqStatus[self[1]][nextToSent[self[1]].IR] := IR_SENT;
                    end if;
                    
                    \* ControllerThreadForwardIR consists of 2 operations;
                    \* 1. Forwarding the IR to the switch if the OFC is the master in NIB
                    \* 2. Updating the state on db to SENT_DONE
                    \* Worker may fail between these operations
                    ControllerThreadForwardIR:
                        controllerWaitForLockFree(); 
                        await ~isProcTerminated(self[1], self[2]);
                        \* since we pass the lock to the switch after 
                        whichStepToFail(2);
                        if (stepOfFailure = 1) then
                            controllerModuleFails();
                            goto ControllerThreadStateReconciliation;
                            ...
                            \* added a label below to allow race condition between packets transmitted from different
                            \* controllers 
                            ControllerWorkerIRTransmitted: 
                            if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} \/ nextToSent[self[1]].type # INSTALL_FLOW then 
                                controllerSendIR(nextToSent[self[1]]);
                            end if;
                            if (stepOfFailure = 2) then
                                controllerModuleFails();
                                goto ControllerThreadStateReconciliation;
        ...
        \*    controllerModuleFailOrNot();
        \*    if (controllerSubmoduleFailStat[self] = NotFailed) then 
        \*        await ~isSwitchSuspended(nextToSent[self[1]].to);
        \*    else
        \*        goto ControllerThreadStateReconciliation;
        \*    end if;
        ...
        \*    controllerModuleFailOrNot();
        \*    if (controllerSubmoduleFailStat[self] = NotFailed) then
        \*        if workerShouldFastRecovery(self[1], nextToSent[self[1]]) then
        \*            controllerStateNIB[self] := [type |-> STATUS_LOCKING, next |-> nextToSent[self[1]], index |-> entryIndex]; 
        ...
        \* 3. Remove the IR from the tagged buffer (Lazy removel strategy) 
        ...
        RemoveFromScheduledIRSet:
            ...
            if (stepOfFailure # 1) then 
                ...
                \* assert nextToSent[self[1]] \in SetScheduledIRs[IR2SW[nextIRToSent]];
                if nextToSent[self[1]].type = INSTALL_FLOW then
                    \* only master OFC has write access to SetScheduledIRs
                    if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then 
                        SetScheduledIRs[nextToSent[self[1]].to] := SetScheduledIRs[nextToSent[self[1]].to]\{nextToSent[self[1]].IR};
                    end if;
                end if;
                ...
                if (stepOfFailure # 2) then  
                    ...
                    if (stepOfFailure # 3) then
                        ...
                        modifiedRemove(workerLocalQueue[self[1]], nextToSent[self[1]]);
                        if nextToSent[self[1]].type = INSTALL_FLOW then \* TODO(@Pooria): maybe have to add label here. 
                            \* ony master OFC has write access to IR Queue
                            if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then 
                                removeItem(IRQueueNIB, nextToSent[self[1]]);
                            end if;
        ...
        if (controllerStateNIB[self].type = STATUS_LOCKING) then
            if (controllerStateNIB[self].next.type = INSTALL_FLOW) then
                if (IRStatus[controllerStateNIB[self].next.IR] = IR_SENT) then
                    \* only master OFC has write access to IR Status in NIB
                        if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then 
         ...
         if shouldSuspendSw(monitoringEvent) /\ SwSuspensionStatus[monitoringEvent.swID] = SW_RUN then
            ...
            \* if masterState[self[1]] = ROLE_MASTER then
                ControllerSuspendSW: 
                    ...
                    controllerModuleFailOrNot();
                    if (controllerSubmoduleFailStat[self] = NotFailed) then
                        \* only master OFC can modify the topology
                        if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then
                        ...
                        ResetAllIRs:
                            while TRUE do \* Initially: while setIRsToReset # {} do
                                ...
                                if (controllerSubmoduleFailStat[self] = NotFailed) then
                                    ...
                                    \* only master OFC has write access to IR Status
                                    if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then 
    ...
    while ~shouldMonitoringServerTerminate(self[1]) do
        ...
        assert \/ /\ msg.type = INSTALLED_SUCCESSFULLY
               ...
               \/ msg.type \in {ROLE_REPLY, BAD_REQUEST, FLOW_STAT_REPLY};
        ...
        (*    if msg.type = RECEIVED_SUCCESSFULLY then 
            ...
            if msg.type = INSTALLED_SUCCESSFULLY then
                ...
                \* make sure the reply is for an IR with reconcile flag. then;
                \* 1. if entry found, set the flag of IR to IR_DONE
                \* 2. if entry not found, add the IR to reschedule set which is used by NIB event handler
                \* to reschedule the IRs that are not installed. 
                UpdateReconcileIR:
                    assert msg.status \in {NO_ENTRY, ENTRY_FOUND};
                    assert msg.IR \in 1..MAX_NUM_IRs;
                    assert IRStatus[msg.IR] \in {IR_DONE, IR_RECONCILE};
                    await ~isProcTerminated(self[1], self[2]);
                    controllerWaitForLockFree(); 
                    controllerModuleFailOrNot();
                    
                    if msg.status = ENTRY_FOUND then
                        \* only master OFC has write access to IR status
                        if masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} then 
                            IRStatus[msg.IR] := IR_DONE;
                        end if;
                        flowStatReqStatus[self[1]][msg.IR] := IR_DONE;
                    else \* when msg.status = NO_ENTRY   
                        flowStatReqStatus[self[1]][msg.IR] := IR_DONE;
                        setIRToReschedule[self[1]] := setIRToReschedule[self[1]] \cup {msg.IR}
                    end if;
    ...
    ofcFailoverChangeRoles:
        ...
        \* ROLE_MASTER_INIT 
        masterState[ofc0] := ROLE_SLAVE_INIT || masterState[ofc1] := ROLE_MASTER_INIT;
    end process    
    \* ==========================
       
    end algorithm
*)
\* BEGIN TRANSLATION - the hash of the PCal code: PCal-9222c55dfe110a8bf94ca40bfd6906c2
\* Process variable stepOfFailure of process controllerSequencer at line 1507 col 50 changed to stepOfFailure_
\* Process variable stepOfFailure of process controllerWorkerThreads at line 1721 col 29 changed to stepOfFailure_c
VARIABLES switchLock, controllerLock, FirstInstall, sw_fail_ordering_var, 
          ...
          workerLocalQueue, nextToSent, controllerRoleInSW, nibEventQueue, 
          roleUpdateStatus, flowStatReqStatus, isOfcEnabled, 
          setScheduledRoleUpdates, ofcModuleTerminationStatus, 
          setIRToReschedule, masterState, controllerStateNIB, IRStatus, 
          SwSuspensionStatus, IRQueueNIB, subscribeList, SetScheduledIRs, 
          ofcFailoverStateNIB, pc
...
VARIABLES ingressPkt, ingressIR, egressMsg, ofaInMsg, ofaOutConfirmation, 
          ...
          stepOfFailure_, subscriberOfcSet, ofcID, event, irSet, currIR, 
          currSW, swSet, IRQueueEntries, entry, entryIndex, rowIndex, 
...
vars == << switchLock, controllerLock, FirstInstall, sw_fail_ordering_var, 
           ...
           workerThreadRanking, workerLocalQueue, nextToSent, 
           controllerRoleInSW, nibEventQueue, roleUpdateStatus, 
           flowStatReqStatus, isOfcEnabled, setScheduledRoleUpdates, 
           ofcModuleTerminationStatus, setIRToReschedule, masterState, 
...
Init == (* Global variables *)
        ...
        /\ irSet = [self \in ({ofc0, ofc1} \X {NIB_EVENT_HANDLER}) |-> {}]
        /\ currIR = [self \in ({ofc0, ofc1} \X {NIB_EVENT_HANDLER}) |-> 0]
        ...
        /\ pc = [self \in ProcSet |-> CASE self \in ({SW_SIMPLE_ID} \X SW) -> "SwitchSimpleProcess"
                                        ...
                                        [] self \in ( {"proc"} \X {OFC_FAILOVER}) -> "ofcFailoverChangeRoles"]
...
SwitchSimpleProcess(self) == /\ pc[self] = "SwitchSimpleProcess"
                             ...
                             /\ Assert(ingressPkt'[self].type \in {INSTALL_FLOW, ROLE_REQ, FLOW_STAT_REQ}, 
                                       "Failure of assertion at line 1129, column 9.")
                             /\ IF ingressPkt'[self].type = INSTALL_FLOW
                                   ...
                                   ELSE /\ IF ingressPkt'[self].type = ROLE_REQ
                                              THEN /\ Assert(ingressPkt'[self].roletype \in {ROLE_MASTER, ROLE_SLAVE, ROLE_EQUAL}, 
                                                             "Failure of assertion at line 1139, column 13.")
                                              ...
                                              ELSE /\ IF ingressPkt'[self].type = FLOW_STAT_REQ
                                                         THEN /\ Assert(ingressPkt'[self].type \in 1..MAX_NUM_IRs, 
                                                                        "Failure of assertion at line 1145, column 13.")
                                                              /\ IF existMatchingEntryTCAM(self[2], ingressPkt'[self].IR)
                                                                    THEN /\ Ofa2NicAsicBuff' = [Ofa2NicAsicBuff EXCEPT ![self[2]] = Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                                                                                                                      from |-> self[2],
                                                                                                                                                                      to |-> (ingressPkt'[self].from),
                                                                                                                                                                      status |-> ENTRY_FOUND,
                                                                                                                                                                      IR |-> (ingressPkt'[self].IR)])]
                                                                    ELSE /\ Ofa2NicAsicBuff' = [Ofa2NicAsicBuff EXCEPT ![self[2]] = Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                                                                                                                      from |-> self[2],
                                                                                                                                                                      to |-> (ingressPkt'[self].from),
                                                                                                                                                                      status |-> NO_ENTRY,
                                                                                                                                                                      IR |-> (ingressPkt'[self].IR)])]
                             ...
                             /\ Assert(\/ switchLock[2] = self[2]
                                       \/ switchLock[2] = NO_LOCK, 
                                       "Failure of assertion at line 924, column 9 of macro called at line 1152, column 9.")
                             /\ switchLock' = <<NO_LOCK, NO_LOCK>>
                             ...
                             /\ UNCHANGED << controllerLock, FirstInstall, 
                                             ...
                                             idThreadWorkingOnIR, 
                                             workerThreadRanking, 
                                             workerLocalQueue, nextToSent, 
                                             controllerRoleInSW, nibEventQueue, 
                                             roleUpdateStatus, 
                                             flowStatReqStatus, isOfcEnabled, 
                                             setScheduledRoleUpdates, 
                                             ofcModuleTerminationStatus, 
                                             setIRToReschedule, masterState, 
                                             ...
                                             ofcID, event, irSet, currIR, 
                                             currSW, swSet, IRQueueEntries, 
                                             entry, entryIndex, rowIndex, 
                                             rowRemove, removeRow, 
...
SwitchRcvPacket(self) == /\ pc[self] = "SwitchRcvPacket"
                         ...
                         /\ Len(controller2Switch[self[2]]) > 0
                         /\ noPacketInFly(self[2])
                         /\ ingressIR' = [ingressIR EXCEPT ![self] = Head(controller2Switch[self[2]])]
                         /\ Assert(ingressIR'[self].type \in {ROLE_REQ, INSTALL_FLOW, FLOW_STAT_REQ}, 
                         ...
                         /\ UNCHANGED << controllerLock, FirstInstall, 
                                         ...
                                         subscriberOfcSet, ofcID, event, irSet, 
                                         currIR, currSW, swSet, IRQueueEntries, 
                                         entry, entryIndex, rowIndex, 
...
SwitchFromOFAPacket(self) == /\ pc[self] = "SwitchFromOFAPacket"
                             ...
                             /\ \/ switchLock \in {<<NO_LOCK, NO_LOCK>>, self}
                                \/ switchLock[2] = self[2]
                             /\ switchLock' = self
                             /\ Assert(egressMsg'[self].type \in {INSTALLED_SUCCESSFULLY, ROLE_REPLY, BAD_REQUEST, FLOW_STAT_REPLY}, 
...
SwitchNicAsicSendOutMsg(self) == /\ pc[self] = "SwitchNicAsicSendOutMsg"
                                 /\ IF swCanReceivePackets(self[2])
                                       THEN /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                                            ...
                                            /\ IF switchShouldReleaseLock(self[2])
                                                  THEN /\ Assert(\/ switchLock[2] = self[2]
                                                                 \/ switchLock[2] = NO_LOCK, 
                                                                 "Failure of assertion at line 924, column 9 of macro called at line 1215, column 21.")
                                                       /\ switchLock' = <<NO_LOCK, NO_LOCK>>
                                                  ELSE /\ TRUE
                                                       /\ UNCHANGED switchLock
                                 ...
                                 /\ UNCHANGED << controllerLock, FirstInstall, 
                                                 ...
                                                 stepOfFailure_, 
                                                 subscriberOfcSet, ofcID, 
                                                 event, irSet, currIR, currSW, 
                                                 swSet, IRQueueEntries, entry, 
                                                 entryIndex, rowIndex, 
                                                 rowRemove, removeRow, 
                                                 stepOfFailure_c, 
                                                 monitoringEvent, 
...
SwitchOfaProcessPacket(self) == /\ pc[self] = "SwitchOfaProcessPacket"
                                /\ IF swOFACanProcessIRs(self[2])
                                      THEN /\ IF ofaInMsg[self].type = INSTALL_FLOW
                                                 ...
                                                 ELSE /\ IF ofaInMsg[self].type = ROLE_REQ
                                                            THEN /\ Assert(ofaInMsg[self].roletype \in {ROLE_MASTER, ROLE_SLAVE, ROLE_EQUAL}, 
                                                                           "Failure of assertion at line 1281, column 21.")
                                                                 /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                                                                 /\ \/ switchLock \in {<<NO_LOCK, NO_LOCK>>, self}
                                                                    \/ switchLock[2] = self[2]
                                                                 /\ switchLock' = self
                                                            ...
                                                            ELSE /\ IF ofaInMsg[self].type = FLOW_STAT_REQ
                                                                       THEN /\ Assert(ofaInMsg[self].IR \in 1..MAX_NUM_IRs, 
                                                                                      "Failure of assertion at line 1295, column 21.")
                                                                            /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                                                                            /\ \/ switchLock \in {<<NO_LOCK, NO_LOCK>>, self}
                                                                               \/ switchLock[2] = self[2]
                                                                            /\ switchLock' = self
                                                                            /\ IF existMatchingEntryTCAM(self[2], ofaInMsg[self].IR)
                                                                                  THEN /\ Ofa2NicAsicBuff' = [Ofa2NicAsicBuff EXCEPT ![self[2]] = Append(Ofa2NicAsicBuff[self[2]], [type |-> FLOW_STAT_REPLY,
                                                                                                                                                                                    from |-> self[2],
                                                                                                                                                                                    to |-> (ofaInMsg[self].from),
                                                                                                                                                                                    status |-> ENTRY_FOUND,
...
SwitchOfaProcOut(self) == /\ pc[self] = "SwitchOfaProcOut"
                          ...
                          /\ Assert(ofaOutConfirmation'[self].IR \in 1..MAX_NUM_IRs, 
                                    "Failure of assertion at line 1322, column 9.")
                          ...
                          /\ UNCHANGED << controllerLock, FirstInstall, 
                                          ...
                                          workerLocalQueue, nextToSent, 
                                          controllerRoleInSW, nibEventQueue, 
                                          roleUpdateStatus, flowStatReqStatus, 
                                          isOfcEnabled, 
                                          setScheduledRoleUpdates, 
                                          ofcModuleTerminationStatus, 
                                          setIRToReschedule, masterState, 
                                          ...
                                          toBeScheduledIRs, nextIR, 
                                          stepOfFailure_, subscriberOfcSet, 
                                          ofcID, event, irSet, currIR, currSW, 
                                          swSet, IRQueueEntries, entry, 
...
SwitchInstallerProc(self) == /\ pc[self] = "SwitchInstallerProc"
                             ...
                             /\ Assert(installerInIR'[self].IR \in 1..MAX_NUM_IRs, 
                                       "Failure of assertion at line 1358, column 8.")
                             ...
                             /\ UNCHANGED << controllerLock, FirstInstall, 
                                             ...
                                             subscriberOfcSet, ofcID, event, 
                                             irSet, currIR, currSW, swSet, 
                                             IRQueueEntries, entry, entryIndex, 
...
SwitchFailure(self) == /\ pc[self] = "SwitchFailure"
                       ...
                       /\ Assert((self[2]) \in Head(sw_fail_ordering_var), 
                                 "Failure of assertion at line 591, column 9 of macro called at line 1410, column 9.")
                       ...
                       /\ IF failedElem'[self] = "cpu"
                             THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                  /\ Assert(switchStatus[self[2]].cpu = NotFailed, 
                                            "Failure of assertion at line 670, column 9 of macro called at line 1419, column 17.")
                             ...
                             ELSE /\ IF failedElem'[self] = "ofa"
                                        THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                             /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].ofa = NotFailed, 
                                                       "Failure of assertion at line 723, column 9 of macro called at line 1422, column 17.")
                                        ...
                                        ELSE /\ IF failedElem'[self] = "installer"
                                                   THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                                        /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].installer = NotFailed, 
                                                                  "Failure of assertion at line 768, column 9 of macro called at line 1425, column 17.")
                                                        ...
                                                        /\ IF switchStatus'[self[2]].nicAsic = NotFailed /\ switchStatus'[self[2]].ofa = NotFailed
                                                              THEN /\ Assert(switchStatus'[self[2]].installer = Failed, 
                                                                             "Failure of assertion at line 772, column 13 of macro called at line 1425, column 17.")
                                                   ...
                                                   ELSE /\ IF failedElem'[self] = "nicAsic"
                                                              THEN /\ pc[<<SW_RESOLVE_PROC, self[2]>>] = "SwitchResolveFailure"
                                                                   /\ Assert(switchStatus[self[2]].nicAsic = NotFailed, 
                                                                             "Failure of assertion at line 615, column 9 of macro called at line 1428, column 17.")
                                                                   ...
                                                                   /\ swSeqChangedStatus' = [swSeqChangedStatus EXCEPT ![getMasterController(self[2])] = Append(swSeqChangedStatus[getMasterController(self[2])], statusMsg'[self])]
                                                              ELSE /\ Assert(FALSE, 
                                                                             "Failure of assertion at line 1429, column 14.")
                                                                   /\ UNCHANGED << swSeqChangedStatus, 
                                                                                   controller2Switch, 
                                                                                   switchStatus, 
                                                                                   swFailedNum, 
                       ...
                       /\ UNCHANGED << switchLock, controllerLock, 
                                       ...
                                       event, irSet, currIR, currSW, swSet, 
                                       IRQueueEntries, entry, entryIndex, 
                                       rowIndex, rowRemove, removeRow, 
                                       stepOfFailure_c, monitoringEvent, 
...
SwitchResolveFailure(self) == /\ pc[self] = "SwitchResolveFailure"
                              ...
                              /\ IF recoveredElem'[self] = "cpu"
                                    THEN /\ ofaStartingMode(self[2]) /\ installerInStartingMode(self[2])
                                         /\ Assert(switchStatus[self[2]].cpu = Failed, 
                                                   "Failure of assertion at line 698, column 9 of macro called at line 1455, column 39.")
                                    ...
                                    ELSE /\ IF recoveredElem'[self] = "nicAsic"
                                               THEN /\ nicAsicStartingMode(self[2])
                                                    /\ Assert(switchStatus[self[2]].nicAsic = Failed, 
                                                              "Failure of assertion at line 642, column 9 of macro called at line 1456, column 46.")
                                               ...
                                               ELSE /\ IF recoveredElem'[self] = "ofa"
                                                          THEN /\ ofaStartingMode(self[2])
                                                               /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].ofa = Failed, 
                                                                         "Failure of assertion at line 746, column 9 of macro called at line 1457, column 42.")
                                                          ...
                                                          ELSE /\ IF recoveredElem'[self] = "installer"
                                                                     THEN /\ installerInStartingMode(self[2])
                                                                          /\ Assert(switchStatus[self[2]].cpu = NotFailed /\ switchStatus[self[2]].installer = Failed, 
                                                                                    "Failure of assertion at line 791, column 9 of macro called at line 1458, column 48.")
                                                                          /\ switchStatus' = [switchStatus EXCEPT ![self[2]].installer = NotFailed]
                                                                          /\ IF switchStatus'[self[2]].nicAsic = NotFailed /\ switchStatus'[self[2]].ofa = NotFailed
                                                                                THEN /\ Assert(switchStatus'[self[2]].installer = NotFailed, 
                                                                                               "Failure of assertion at line 794, column 13 of macro called at line 1458, column 48.")
                                                                                     /\ controlMsgCounter' = [controlMsgCounter EXCEPT ![self[2]] = controlMsgCounter[self[2]] + 1]
                                                                                     /\ statusResolveMsg' = [statusResolveMsg EXCEPT ![self] = [type |-> KEEP_ALIVE,
                                                                                                                                                swID |-> self[2],
                                                                                                                                                num |-> controlMsgCounter'[self[2]],
                                                                                ...
                                                                                ELSE /\ TRUE
                                                                                     /\ UNCHANGED << swSeqChangedStatus, 
                                                                                                     ...
                                                                                                     statusResolveMsg >>
                                                                     ELSE /\ Assert(FALSE, 
                                                                                    "Failure of assertion at line 1459, column 14.")
                                                                          /\ UNCHANGED << swSeqChangedStatus, 
                                                                                          switchStatus, 
                                                                                          controlMsgCounter, 
                                                                                          statusResolveMsg >>
...
ghostProc(self) == /\ pc[self] = "ghostProc"
                   ...
                   /\ Assert(\/ switchLock[2] = switchLock[2]
                             \/ switchLock[2] = NO_LOCK, 
                             "Failure of assertion at line 924, column 9 of macro called at line 1493, column 9.")
                   /\ switchLock' = <<NO_LOCK, NO_LOCK>>
                   ...
                   /\ UNCHANGED << controllerLock, FirstInstall, 
                                   ...
                                   stepOfFailure_, subscriberOfcSet, ofcID, 
                                   event, irSet, currIR, currSW, swSet, 
                                   IRQueueEntries, entry, entryIndex, rowIndex, 
                                   rowRemove, removeRow, stepOfFailure_c, 
...
SchedulerMechanism(self) == /\ pc[self] = "SchedulerMechanism"
                            ...
                            /\ UNCHANGED << switchLock, controllerLock, 
                                            ...
                                            failedElem, failedSet, 
                                            statusResolveMsg, recoveredElem, 
                                            toBeScheduledIRs, subscriberOfcSet, 
                                            ofcID, event, irSet, currIR, 
                                            currSW, swSet, IRQueueEntries, 
                                            entry, entryIndex, rowIndex, 
                                            rowRemove, removeRow, 
...
NibEventHandlerProc(self) == /\ pc[self] = "NibEventHandlerProc"
                             /\ \/ masterState[self[1]] = ROLE_MASTER_INIT
                                \/ masterState[self[1]] = ROLE_SLAVE_INIT
                                \/ setIRToReschedule[self[1]] # {}
                                \/ /\ masterState[self[1]] = ROLE_MASTER
                                   /\ nibEventQueue[self[1]] # <<>>
                             /\ IF masterState[self[1]] = ROLE_SLAVE_INIT
                                   THEN /\ subscribeList' = [subscribeList EXCEPT !.IRQueueNIB = subscribeList.IRQueueNIB \ {self[1]}]
                                        /\ ofcModuleTerminationStatus' = [ofcModuleTerminationStatus EXCEPT ![self[1]] = [y \in (CONTROLLER_THREAD_POOL \cup {CONT_MONITOR, CONT_EVENT}) |-> TERMINATE_UNGRACEFULLY]]
                                        /\ ofcFailoverStateNIB' = [ofcFailoverStateNIB EXCEPT ![self[1]] = FAILOVER_TERMINATE_DONE]
                                        /\ masterState' = [masterState EXCEPT ![self[1]] = ROLE_SLAVE]
                                        ...
                                        /\ UNCHANGED << workerLocalQueue, 
                                                        ...
                                                        currIR, swSet >>
                                   ELSE /\ IF masterState[self[1]] = ROLE_MASTER_INIT
                                              THEN /\ isOfcEnabled' = [isOfcEnabled EXCEPT ![self[1]] = TRUE]
                                                   /\ swSet' = [swSet EXCEPT ![self] = getSetSwitchInSlaveRole(self[1]) \cup getSetSwitchInEqualRole(self[1])]
                                                   /\ pc' = [pc EXCEPT ![self] = "ScheduleRoleUpdateMaster"]
                                                   /\ UNCHANGED << workerLocalQueue, 
                                                                   nibEventQueue, 
                                                                   flowStatReqStatus, 
                                                                   setIRToReschedule, 
                                                                   ...
                                                                   event, 
                                                                   currIR >>
                                              ELSE /\ IF setIRToReschedule[self[1]] # {}
                                                         THEN /\ currIR' = [currIR EXCEPT ![self] = CHOOSE x \in setIRToReschedule[self[1]]: TRUE]
                                                              /\ Assert(IRStatus[currIR'[self]] = IR_RECONCILE, 
                                                                        "Failure of assertion at line 1694, column 17.")
                                                              /\ SetScheduledIRs' = [SetScheduledIRs EXCEPT ![IR2SW[currIR'[self]]] = SetScheduledIRs[IR2SW[currIR'[self]]] \cup {currIR'[self]}]
                                                              /\ IRStatus' = [IRStatus EXCEPT ![currIR'[self]] = IR_NONE]
                                                              /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][currIR'[self]] = IR_NONE]
                                                              /\ workerLocalQueue' = [workerLocalQueue EXCEPT ![self[1]] = Append((workerLocalQueue[self[1]]), [item |-> ([type |-> INSTALL_FLOW,
                                                                                                                                                                          to |-> IR2SW[currIR'[self]],
                                                                                                                                                                          IR |-> currIR'[self]]), id |-> -1, tag |-> NO_TAG])]
                                                              /\ setIRToReschedule' = [setIRToReschedule EXCEPT ![self[1]] = setIRToReschedule[self[1]] \ {currIR'[self]}]
                                                              /\ UNCHANGED << nibEventQueue, 
                                                                              event >>
                             ...
                             /\ UNCHANGED << switchLock, controllerLock, 
                                             ...
                                             IR2SW, irCounter, MAX_IR_COUNTER, 
                                             idThreadWorkingOnIR, 
                                             workerThreadRanking, nextToSent, 
                                             controllerRoleInSW, 
                                             roleUpdateStatus, 
                                             setScheduledRoleUpdates, 
                                             controllerStateNIB, 
...
ScheduleRoleUpdateMaster(self) == /\ pc[self] = "ScheduleRoleUpdateMaster"
                                  /\ IF swSet[self] # {}
                                        THEN /\ currSW' = [currSW EXCEPT ![self] = CHOOSE x \in swSet[self]: TRUE]
                                             ...
                                             /\ pc' = [pc EXCEPT ![self] = "ScheduleRoleUpdateMaster"]
                                             /\ irSet' = irSet
                                        ELSE /\ allSwitchInMasterRole(self[1])
                                             /\ irSet' = [irSet EXCEPT ![self] = getSetIrReconciliation]
                                             /\ pc' = [pc EXCEPT ![self] = "ScheduleFlowStatReq"]
                                             /\ UNCHANGED << workerLocalQueue, 
                                                             currSW, swSet >>
                                  /\ UNCHANGED << switchLock, controllerLock, 
                                                  FirstInstall, 
...
ScheduleFlowStatReq(self) == /\ pc[self] = "ScheduleFlowStatReq"
                             /\ IF irSet[self] # {}
                                   THEN /\ currIR' = [currIR EXCEPT ![self] = CHOOSE x \in irSet[self]: TRUE]
                                        /\ IRStatus' = [IRStatus EXCEPT ![currIR'[self]] = IR_RECONCILE]
                                        /\ workerLocalQueue' = [workerLocalQueue EXCEPT ![self[1]] = Append((workerLocalQueue[self[1]]), [item |-> ([type |-> FLOW_STAT_REQ,
                                                                                                                                                     to |-> IR2SW[currIR'[self]],
                                                                                                                                                     IR |-> currIR'[self]]), id |-> -1, tag |-> NO_TAG])]
                                        /\ irSet' = [irSet EXCEPT ![self] = irSet[self] \ {currIR'[self]}]
                                        /\ pc' = [pc EXCEPT ![self] = "ScheduleFlowStatReq"]
                                        /\ UNCHANGED << subscribeList, 
                                                        IRQueueEntries >>
...
QueryIRQueueNIB(self) == /\ pc[self] = "QueryIRQueueNIB"
                         /\ IF IRQueueEntries[self] # <<>>
                               THEN /\ entry' = [entry EXCEPT ![self] = Head(IRQueueEntries[self])]
                                    ...
                                    /\ pc' = [pc EXCEPT ![self] = "QueryIRQueueNIB"]
                                    /\ UNCHANGED masterState
                               ELSE /\ masterState' = [masterState EXCEPT ![self[1]] = ROLE_MASTER]
                                    /\ pc' = [pc EXCEPT ![self] = "NibEventHandlerProc"]
                                    /\ UNCHANGED << workerLocalQueue, 
                                                    IRQueueEntries, entry >>
                         /\ UNCHANGED << switchLock, controllerLock, 
                                         ...
                                         ofcModuleTerminationStatus, 
                                         setIRToReschedule, controllerStateNIB, 
                                         IRStatus, SwSuspensionStatus, 
                                         IRQueueNIB, subscribeList, 
                                         SetScheduledIRs, ofcFailoverStateNIB, 
                                         ingressPkt, ingressIR, egressMsg, 
...
nibEventHandler(self) == NibEventHandlerProc(self)
                            \/ ScheduleRoleUpdateMaster(self)
                            \/ ScheduleFlowStatReq(self)
...
ControllerThread(self) == /\ pc[self] = "ControllerThread"
                          /\ IF ~shouldWorkerTerminate(self[1], self[2])
                                THEN /\ isOfcEnabled[self[1]]
                                     ...
                                     /\ ~isProcTerminated(self[1], self[2])
                                     /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                                     /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                     /\ rowIndex' = [rowIndex EXCEPT ![self] = getFirstIRIndexToRead((workerLocalQueue[self[1]]), self)]
                                     /\ nextToSent' = [nextToSent EXCEPT ![self[1]] = (workerLocalQueue[self[1]])[rowIndex'[self]].item]
                                     /\ IF existEquivalentItemWithID((workerLocalQueue[self[1]]), (nextToSent'[self[1]]))
                                           THEN /\ entryIndex' = [entryIndex EXCEPT ![self] = getIdOfEquivalentItem((workerLocalQueue[self[1]]), (nextToSent'[self[1]]))]
                                                /\ UNCHANGED irCounter
                                     ...
                                     /\ IF (stepOfFailure_c'[self] = 1)
                                           THEN /\ controllerSubmoduleFailStat' = [controllerSubmoduleFailStat EXCEPT ![self] = Failed]
                                                ...
                                                /\ UNCHANGED << idThreadWorkingOnIR, 
                                                                controllerStateNIB >>
                                           ELSE /\ controllerStateNIB' = [controllerStateNIB EXCEPT ![self] = [type |-> STATUS_LOCKING, next |-> nextToSent'[self[1]], index |-> entryIndex'[self]]]
                                                /\ IF (stepOfFailure_c'[self] = 2)
                                                      ...
                                                      ELSE /\ IF idThreadWorkingOnIR[self[1]][entryIndex'[self]] = IR_UNLOCK
                                                                 THEN /\ threadWithLowerIDGetsTheLock(self[1], self, nextToSent'[self[1]], workerLocalQueue'[self[1]])
                                ...
                                /\ IF (controllerSubmoduleFailStat[self] = NotFailed /\
                                      ...
                                      ELSE /\ TRUE
                                           /\ UNCHANGED << controllerSubmoduleFailNum, 
                                                           controllerSubmoduleFailStat >>
                                /\ IF (controllerSubmoduleFailStat'[self] = NotFailed)
                                      THEN /\ IF workerCanSendTheIR(self[1], nextToSent[self[1]])
                                                 THEN /\ Assert(nextToSent[self[1]].type \in {INSTALL_FLOW, ROLE_REQ, FLOW_STAT_REQ}, 
                                                                "Failure of assertion at line 1791, column 21.")
                                                      /\ IF nextToSent[self[1]].type = INSTALL_FLOW
                                                            THEN /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT}
                                                                       THEN /\ IRStatus' = [IRStatus EXCEPT ![nextToSent[self[1]].IR] = IR_SENT]
                                                                       ELSE /\ TRUE
                                                                            /\ UNCHANGED IRStatus
                                                                 /\ UNCHANGED << roleUpdateStatus, 
                                                                                 flowStatReqStatus >>
                                                            ELSE /\ IF nextToSent[self[1]].type = ROLE_REQ
                                                                       THEN /\ roleUpdateStatus' = [roleUpdateStatus EXCEPT ![self[1]][nextToSent[self[1]].to] = IR_SENT]
                                                                            /\ UNCHANGED flowStatReqStatus
                                                                       ELSE /\ IF nextToSent[self[1]].type = FLOW_STAT_REQ
                                                                                  THEN /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][nextToSent[self[1]].IR] = IR_SENT]
                                                                                  ELSE /\ TRUE
                                                                                       /\ UNCHANGED flowStatReqStatus
                                                                            /\ UNCHANGED roleUpdateStatus
                                                                 /\ UNCHANGED IRStatus
...
ControllerThreadForwardIR(self) == /\ pc[self] = "ControllerThreadForwardIR"
                                   ...
                                   /\ IF (stepOfFailure_c'[self] = 1)
                                         THEN /\ controllerSubmoduleFailStat' = [controllerSubmoduleFailStat EXCEPT ![self] = Failed]
                                              /\ controllerSubmoduleFailNum' = [controllerSubmoduleFailNum EXCEPT ![self[1]] = controllerSubmoduleFailNum[self[1]] + 1]
                                              /\ pc' = [pc EXCEPT ![self] = "ControllerThreadStateReconciliation"]
                                         ELSE /\ pc' = [pc EXCEPT ![self] = "ControllerWorkerIRTransmitted"]
                                              /\ UNCHANGED << controllerSubmoduleFailNum, 
                                                              controllerSubmoduleFailStat >>
                                   /\ UNCHANGED << switchLock, controllerLock, 
                                                   FirstInstall, 
...
ControllerWorkerIRTransmitted(self) == /\ pc[self] = "ControllerWorkerIRTransmitted"
                                       /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT} \/ nextToSent[self[1]].type # INSTALL_FLOW
                                             THEN /\ IF (nextToSent[self[1]]).type = INSTALL_FLOW
                                                        THEN /\ controller2Switch' = [controller2Switch EXCEPT ![(nextToSent[self[1]]).to] = Append(controller2Switch[(nextToSent[self[1]]).to], [type |-> INSTALL_FLOW,
                                                                                                                                                                                                  from |-> self[1],
                                                                                                                                                                                                  to |-> (nextToSent[self[1]]).to,
                                                                                                                                                                                                  IR |-> (nextToSent[self[1]]).IR])]
                                                        ELSE /\ IF (nextToSent[self[1]]).type = ROLE_REQ
                                                                   THEN /\ controller2Switch' = [controller2Switch EXCEPT ![(nextToSent[self[1]]).to] = Append(controller2Switch[(nextToSent[self[1]]).to], [type |-> ROLE_REQ,
                                                                                                                                                                                                             from |-> self[1],
                                                                                                                                                                                                             to |-> (nextToSent[self[1]]).to,
                                                                                                                                                                                                             roletype |-> (nextToSent[self[1]]).roletype])]
                                                                   ELSE /\ IF (nextToSent[self[1]]).type = FLOW_STAT_REQ
                                                                              THEN /\ controller2Switch' = [controller2Switch EXCEPT ![(nextToSent[self[1]]).to] = Append(controller2Switch[(nextToSent[self[1]]).to], [type |-> FLOW_STAT_REQ,
                                                                                                                                                                                                                        from |-> self[1],
                                                                                                                                                                                                                        to |-> (nextToSent[self[1]]).to,
                                                                                                                                                                                                                        IR |-> (nextToSent[self[1]]).IR])]
                                                                              ELSE /\ TRUE
                                                                                   /\ UNCHANGED controller2Switch
                                                  /\ IF whichSwitchModel((nextToSent[self[1]]).to) = SW_COMPLEX_MODEL
                                                        THEN /\ switchLock' = <<NIC_ASIC_IN, (nextToSent[self[1]]).to>>
                                                        ELSE /\ switchLock' = <<SW_SIMPLE_ID, (nextToSent[self[1]]).to>>
                                             ELSE /\ TRUE
                                                  /\ UNCHANGED << switchLock, 
                                                                  controller2Switch >>
                                       /\ IF (stepOfFailure_c[self] = 2)
                                             THEN /\ controllerSubmoduleFailStat' = [controllerSubmoduleFailStat EXCEPT ![self] = Failed]
                                                  ...
                                                  /\ pc' = [pc EXCEPT ![self] = "ControllerThreadStateReconciliation"]
                                                  /\ UNCHANGED controllerStateNIB
                                             ELSE /\ controllerStateNIB' = [controllerStateNIB EXCEPT ![self] = [type |-> STATUS_SENT_DONE,
                                                                                                                 next |-> nextToSent[self[1]],
...
RemoveFromScheduledIRSet(self) == /\ pc[self] = "RemoveFromScheduledIRSet"
                                  ...
                                  /\ IF (stepOfFailure_c'[self] # 1)
                                        THEN /\ IF nextToSent[self[1]].type = INSTALL_FLOW
                                                   THEN /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT}
                                                              THEN /\ SetScheduledIRs' = [SetScheduledIRs EXCEPT ![nextToSent[self[1]].to] = SetScheduledIRs[nextToSent[self[1]].to]\{nextToSent[self[1]].IR}]
                                                              ELSE /\ TRUE
                                                                   /\ UNCHANGED SetScheduledIRs
                                                   ELSE /\ TRUE
                                                        /\ UNCHANGED SetScheduledIRs
                                             /\ IF (stepOfFailure_c'[self] # 2)
                                                   THEN /\ controllerStateNIB' = [controllerStateNIB EXCEPT ![self] = [type |-> NO_STATUS]]
                                                        /\ IF (stepOfFailure_c'[self] # 3)
                                                              THEN /\ rowRemove' = [rowRemove EXCEPT ![self] = getFirstIndexWith((workerLocalQueue[self[1]]), (nextToSent[self[1]]), self)]
                                                                   /\ workerLocalQueue' = [workerLocalQueue EXCEPT ![self[1]] = removeFromSeq((workerLocalQueue[self[1]]), rowRemove'[self])]
                                                                   /\ IF nextToSent[self[1]].type = INSTALL_FLOW
                                                                         THEN /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT}
                                                                                    THEN /\ IF existsEquivalentItem(IRQueueNIB, (nextToSent[self[1]]))
                                                                                               THEN /\ removeRow' = [removeRow EXCEPT ![self] = CHOOSE x \in DOMAIN IRQueueNIB: isIdenticalElement(IRQueueNIB[x], (nextToSent[self[1]]))]
...
ControllerThreadRemoveQueue1(self) == /\ pc[self] = "ControllerThreadRemoveQueue1"
                                      ...
                                      /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                                      /\ switchLock = <<NO_LOCK, NO_LOCK>>
                                      /\ rowRemove' = [rowRemove EXCEPT ![self] = getFirstIndexWith((workerLocalQueue[self[1]]), (nextToSent[self[1]]), self)]
                                      /\ workerLocalQueue' = [workerLocalQueue EXCEPT ![self[1]] = removeFromSeq((workerLocalQueue[self[1]]), rowRemove'[self])]
                                      /\ IF nextToSent[self[1]].type = INSTALL_FLOW
                                            THEN /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT}
                                                       THEN /\ IF existsEquivalentItem(IRQueueNIB, (nextToSent[self[1]]))
...
controllerWorkerThreads(self) == ControllerThread(self)
                                    ...
                                    \/ ControllerWorkerIRTransmitted(self)
...
ControllerEventHandlerProc(self) == /\ pc[self] = "ControllerEventHandlerProc"
                                    /\ IF ~shouldEventHandlerTerminate(self[1])
                                          THEN /\ ~isProcTerminated(self[1], self[2])
                                               /\ isOfcEnabled[self[1]]
                                               /\ moduleIsUp(self)
...
ResetAllIRs(self) == /\ pc[self] = "ResetAllIRs"
                     ...
                     /\ IF (controllerSubmoduleFailStat'[self] = NotFailed)
                           THEN /\ resetIR' = [resetIR EXCEPT ![self] = CHOOSE x \in setIRsToReset[self]: TRUE]
                                ...
                                /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT}
                                       ...
                                       /\ IF ~shouldMonitoringServerTerminate(self[1])
                                             THEN /\ ~isProcTerminated(self[1], self[2])
                                                  ...
                                                  /\ Assert(\/ /\ msg'[self].type = INSTALLED_SUCCESSFULLY
                                                            ...
                                                            \/ msg'[self].type \in {ROLE_REPLY, BAD_REQUEST, FLOW_STAT_REPLY}, 
                                                            "Failure of assertion at line 2184, column 9.")
                                                  /\ IF msg'[self].type = INSTALLED_SUCCESSFULLY
                                                        ...
                                                        ELSE /\ IF msg'[self].type = ROLE_REPLY
                                                                   ...
                                                                   ELSE /\ IF msg'[self].type = FLOW_STAT_REPLY
                                                                              THEN /\ pc' = [pc EXCEPT ![self] = "UpdateReconcileIR"]
                                                                              ELSE /\ IF msg'[self].type = BAD_REQUEST
                                                                                         THEN /\ ~isProcTerminated(self[1], self[2])
                                                                                              /\ TRUE
                                                                                         ELSE /\ Assert(FALSE, 
                                                                                                        "Failure of assertion at line 2266, column 18.")
...
ControllerUpdateRole(self) == /\ pc[self] = "ControllerUpdateRole"
                              ...
                              /\ UNCHANGED << switchLock, controllerLock, 
                                              ...
                                              workerThreadRanking, 
                                              workerLocalQueue, nextToSent, 
                                              nibEventQueue, flowStatReqStatus, 
                                              isOfcEnabled, 
                                              setScheduledRoleUpdates, 
                                              ofcModuleTerminationStatus, 
                                              setIRToReschedule, masterState, 
...
UpdateReconcileIR(self) == /\ pc[self] = "UpdateReconcileIR"
                           /\ Assert(msg[self].status \in {NO_ENTRY, ENTRY_FOUND}, 
                                     "Failure of assertion at line 2243, column 21.")
                           /\ Assert(msg[self].IR \in 1..MAX_NUM_IRs, 
                                     "Failure of assertion at line 2244, column 21.")
                           /\ Assert(IRStatus[msg[self].IR] \in {IR_DONE, IR_RECONCILE}, 
                                     "Failure of assertion at line 2245, column 21.")
                           /\ ~isProcTerminated(self[1], self[2])
                           /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                           /\ switchLock = <<NO_LOCK, NO_LOCK>>
                           /\ IF (controllerSubmoduleFailStat[self] = NotFailed /\
                                 ...
                                 ELSE /\ TRUE
                                      /\ UNCHANGED << controllerSubmoduleFailNum, 
                                                      controllerSubmoduleFailStat >>
                           /\ IF msg[self].status = ENTRY_FOUND
                                 THEN /\ IF masterState[self[1]] \in {ROLE_MASTER, ROLE_MASTER_INIT}
                                            THEN /\ IRStatus' = [IRStatus EXCEPT ![msg[self].IR] = IR_DONE]
                                            ELSE /\ TRUE
                                                 /\ UNCHANGED IRStatus
                                      /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][msg[self].IR] = IR_DONE]
                                      /\ UNCHANGED setIRToReschedule
                                 ELSE /\ flowStatReqStatus' = [flowStatReqStatus EXCEPT ![self[1]][msg[self].IR] = IR_DONE]
                                      /\ setIRToReschedule' = [setIRToReschedule EXCEPT ![self[1]] = setIRToReschedule[self[1]] \cup {msg[self].IR}]
                                      /\ UNCHANGED IRStatus
                           /\ pc' = [pc EXCEPT ![self] = "MonitoringServerRemoveFromQueue"]
                           /\ UNCHANGED << switchLock, controllerLock, 
                                           FirstInstall, sw_fail_ordering_var, 
...
ControllerWatchDogProc(self) == /\ pc[self] = "ControllerWatchDogProc"
                                ...
                                /\ \E module \in controllerFailedModules'[self]:
                                     /\ Assert(controllerSubmoduleFailStat[module] = Failed, 
                                               "Failure of assertion at line 2307, column 13.")
                                     /\ controllerLock' = module
...
ofcFailoverChangeRoles(self) == /\ pc[self] = "ofcFailoverChangeRoles"
                                /\ SHOULD_FAILOVER = 1
                                /\ masterState' = [masterState EXCEPT ![ofc0] = ROLE_SLAVE_INIT,
                                                                      ![ofc1] = ROLE_MASTER_INIT]
                                /\ pc' = [pc EXCEPT ![self] = "Done"]
                                /\ UNCHANGED << switchLock, controllerLock, 
                                                FirstInstall, 
                                                sw_fail_ordering_var, 
...
failoverProc(self) == ofcFailoverChangeRoles(self)
...
\* END TRANSLATION - the hash of the generated TLA code (remove to silence divergence warnings): TLA-100c5b62ae3f3febc30ed12f5c6f2ba1
...
\* Last modified Mon Dec 07 22:43:27 PST 2020 by root
