...
\*                            goto RCComputeNextIR2Schedule;
                            debug := ~allIRsInDAGInstalled(self[1], currDAG.dag) /\ ~isDAGStale(currDAG.id);
...
\* Process variable transaction of process controllerSequencer at line 2046 col 17 changed to transaction_
\* Process variable stepOfFailure of process controllerSequencer at line 2047 col 17 changed to stepOfFailure_c
\* Process variable op1 of process controllerSequencer at line 2051 col 17 changed to op1_
\* Process variable transaction of process OFCFailoverProc at line 2341 col 15 changed to transaction_O
\* Process variable NIBMsg of process OFCNIBEventHandler at line 2405 col 15 changed to NIBMsg_O
\* Process variable stepOfFailure of process controllerWorkerThreads at line 2436 col 64 changed to stepOfFailure_co
\* Process variable transaction of process controllerWorkerThreads at line 2437 col 15 changed to transaction_c
\* Process variable op_ir_status_change of process controllerWorkerThreads at line 2439 col 15 changed to op_ir_status_change_
\* Process variable transaction of process controllerEventHandler at line 2538 col 47 changed to transaction_co
...
Init == (* Global variables *)
        ...
        /\ pc = [self \in ProcSet |-> CASE self \in ({SW_SIMPLE_ID} \X SW) -> "SwitchSimpleProcess"
                                        ...
                                        [] self \in ({rc0} \X {CONT_WORKER_SEQ}) -> "RCSendReadTransaction"
                                     ...
                                     /\ IF TEEventQueue[self[1]] # <<>>
                                           THEN /\ controllerLock = <<NO_LOCK, NO_LOCK>>
                                                ...
                                                /\ Assert(topoChangeEvent'[self].type \in {TOPO_MOD}, 
                                                          "Failure of assertion at line 1934, column 17.")
                                  ...
                                  /\ IF isRCFailed(self)
                                        ...
                                        ELSE /\ NIBUpdateForRC
                                             ...
                                             /\ IF toBeScheduledIRs'[self] = {}
                                                   THEN /\ debug' = [debug EXCEPT ![self] = ~allIRsInDAGInstalled(self[1], currDAG[self].dag) /\ ~isDAGStale(currDAG[self].id)]
                                                        /\ pc' = [pc EXCEPT ![self] = "ControllerWorkerSeqScheduleDAG"]
                                     ...
                                     ELSE /\ NIB2OFC # <<>>
                                          ...
                                          /\ IF localIRSet'[self] # NIBIRSet'[self]
                                                THEN /\ IRQueueOFC' = SelectSeq(NIBMsg_O'[self].value.IRQueueNIB, LAMBDA i: IRStatusOFC[i.IR] = IR_NONE)
                                                     /\ Assert(Len(IRQueueOFC') <= MaxNumIRs, 
                                                               "Failure of assertion at line 2423, column 21.")
                                                ELSE /\ TRUE
...
ControllerThreadForwardIRInner(self) == /\ pc[self] = "ControllerThreadForwardIRInner"
                                        ...
                                        /\ IF isOFCFailed(self)
                                              ...
                                              ELSE /\ Assert(irTypeMapping[nextIRToSent[self]].type \in {INSTALL_FLOW, DELETE_FLOW}, 
                                                             "Failure of assertion at line 1048, column 9 of macro called at line 2494, column 17.")
                                                   /\ Assert(irTypeMapping[nextIRToSent[self]].flow \in 1..MaxNumFlows, 
                                                             "Failure of assertion at line 1049, column 9 of macro called at line 2494, column 17.")
...
ControllerEventHandlerProc(self) == /\ pc[self] = "ControllerEventHandlerProc"
                                    ...
                                    /\ IF shouldSuspendSw(monitoringEvent'[self]) /\ SwSuspensionStatusOFC[monitoringEvent'[self].swID] = SW_RUN
                                          ...
                                          ELSE /\ Assert(FALSE, 
                                                         "Failure of assertion at line 2582, column 13.")
...
OFCMonitorCheckIfMastr(self) == /\ pc[self] = "OFCMonitorCheckIfMastr"
                                ...
                                /\ Assert(msg'[self].type \in {DELETED_SUCCESSFULLY, INSTALLED_SUCCESSFULLY, FLOW_STAT_REPLY}, 
                                          "Failure of assertion at line 2648, column 9.")
                                /\ IF msg'[self].type \in {INSTALLED_SUCCESSFULLY}
                                      THEN /\ irID' = [irID EXCEPT ![self] = getIRIDForFlow(msg'[self].flow, msg'[self].type)]
                                           /\ Assert(msg'[self].from = IR2SW[irID'[self]], 
                                                     "Failure of assertion at line 2655, column 17.")
                                      ...
                                      ELSE /\ IF msg'[self].type \in {DELETED_SUCCESSFULLY}
                                                 THEN /\ irID' = [irID EXCEPT ![self] = getRemovedIRID(msg'[self].flow)]
                                                      /\ Assert(msg'[self].from = IR2SW[removedIR[self]], 
                                                                "Failure of assertion at line 2682, column 13.")
                                                      /\ pc' = [pc EXCEPT ![self] = "OFCUpdateIRNone"]
                                                 ELSE /\ Assert(FALSE, 
                                                                "Failure of assertion at line 2705, column 13.")
...
\* Last modified Mon Jun 21 10:15:41 PDT 2021 by zmy
